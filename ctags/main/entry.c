...
/*  These header files provide for the functions necessary to do file  truncation. */
#ifdef HAVE_FCNTL_H
# include <fcntl.h>
#endif
#ifdef HAVE_IO_H
...
extern void makeFileTag(const char *const fileName)
{
	...
	{
		kindDefinition *kind = getInputLanguageFileKind();
		Assert(kind);
		...
		{
			/* isFieldEnabled is called again in the rendering stage. However,
			 * it is called here for avoiding unnecessary read line loop. */
...
/*  Look through all line beginning with "!_TAG_FILE",
 *  and update those which require it.
 */
...
static bool isTagFile(const char *const filename)
{
	...
	{
		const char *line = readLineRaw(TagFile.vLine, mio);
		ok = (line == NULL) ? true : (bool)(isCtagsLine(line) || isEtagsLine(line));
		mio_free(mio);
	}
	return ok;
}
...
extern void openTagFile(void)
{
	...
	if (TagsToStdout)
	{
		/* Open a tempfile with read and write mode.
		 * Read mode is used when write the result to stdout. */
	...
	else
	{
		...
		bool fileExists = doesFileExist(TagFile.name);
		...
		if (Option.etags)
		{
			TagFile.mio = (Option.append && fileExists)
							? mio_new_file(TagFile.name, "a+b")
							: mio_new_file(TagFile.name, "w+b");
		}
		else
		{
	...
	TagFile.directory = TagsToStdout ? eStrdup(CurrentDirectory)
									 : absoluteDirname(TagFile.name);
}
...
static void writeEtagsIncludes(MIO *const mio)
{
	...
	{
		for (unsigned int i = 0; i < stringListCount(Option.etagsInclude); ++i)
		{
...
extern void closeTagFile(const bool resize)
{
	...
	long desiredSize = mio_tell(TagFile.mio);
	mio_seek(TagFile.mio, 0L, SEEK_END);
	long size = mio_tell(TagFile.mio);
	...
	{
		DebugStatement(debugPrintf(DEBUG_STATUS, "shrinking %s from %ld to %ld bytes\n",
...
static bool isPosSet(MIOPos pos)
{
	...
	for (unsigned int i = 0; i < sizeof(pos); i++)
		r |= p[i];
	return r;
}

extern char *readLineFromBypassAnyway(vString *const vLine,
									  const tagEntryInfo *const tag,
									  long *const pSeekValue)
{
	return (isPosSet(tag->filePosition) || tag->pattern == NULL)
				? readLineFromBypass(vLine, tag->filePosition, pSeekValue)
				: readLineFromBypassSlow(vLine, tag->lineNumber,
										 tag->pattern, pSeekValue);
}

/*  Truncates the text line containing the tag at the character following
 *  the tag, providing a character which designates the end of the tag.
 */
								   ...
								   const char **kind, const char **name)
{
		...
		tag->extensionFields.scopeName != NULL)
	{
		...
		if (name) *name = tag->extensionFields.scopeName;
	}
}
								   ...
								   void *output)
{
	...
	char *line = readLineFromBypass(TagFile.vLine, tag->filePosition, NULL);
	if (line == NULL)
		error(FATAL, "could not read tag line from %s at line %lu",
			  getInputFileName(),tag->lineNumber);
	if (tag->truncateLineAfterTag)
		truncateTagLine(line, tag->name, false);
	
	size_t line_len = strlen(line);
	int searchChar = Option.backward ? '?' : '/';
	const char *terminator = (bool)(line[line_len - 1] == '\n') ? "$": "";
...
extern void attachParserFieldToCorkEntry(int index, fieldType type,
										 const char *value)
{
	if (index == CORK_NIL)
		return;
...
static void copyParserFields(const tagEntryInfo *const tag, tagEntryInfo* slot)
{
	...
	for (unsigned int i = 0; i < tag->usedParserFields; i++)
	{
		...
		attachParserField(slot, tag->parserFields[i].ftype, value);
	}
}
...
static unsigned int queueTagEntry(const tagEntryInfo *const tag)
{
	if (!(TagFile.corkQueue.count < TagFile.corkQueue.length))
	{
		...
		void *tmp = eRealloc(TagFile.corkQueue.queue,
	...
	unsigned int i = TagFile.corkQueue.count;
	TagFile.corkQueue.count++;
	
	tagEntryInfo *slot = TagFile.corkQueue.queue + i;
...
static void initCtagsTag(ctagsTag *tag, const tagEntryInfo *info)
{
	...
	tag->endLineNumber = info->extensionFields.endLine;
						   ...
						   const char *const parserName)
{
	...
	int length = writer->writePtagEntry(TagFile.mio, desc, fileName,
...
extern int makeQualifiedTagEntry(const tagEntryInfo *const e)
{
	...
	if (isXtagEnabled(XTAG_QUALIFIED_TAGS))
	{
		...
		if (e->extensionFields.scopeName)
		{
			...
			char xk = e->extensionFields.scopeKindIndex;
		...
		/* makeExtraTagEntry of c.c doesn't clear scope releated fields. */
#if 0
							 ...
							 long sourceLineNumberDifference)
{
	...
	for (int i = 0; i < PRE_ALLOCATED_PARSER_FIELDS; i++)
