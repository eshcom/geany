...
static int podLineScan(LexAccessor &styler, Sci_PositionU &pos,
	...
	while (pos < endPos) {
		...
		if (IsCRLF(ch)) {
			if (IsCRLF(ch, styler.SafeGetCharAt(pos + 1))) pos++;
			break;
		}
	...
	while (maxSeg > 0) {
		...
		if (!isVar && (maxSeg > 1)) {	// $- or @-specific variable patterns
			...
			if (sc.ch == '$') {
				...
				sLen = 1;
				if (!isPattern && IsSign(c)) {	// @ specials non-pattern
					sLen++;
					isVar = true;
				}
			}
	...
	for (; sc.More(); sc.Forward()) {
		...
		case SCE_PL_IDENTIFIER:		// identifier, bareword, inputsymbol
			if ((!setWord.Contains(sc.ch) && sc.ch != '\'')
				|| sc.Match('.', '.') || sc.chPrev == '>') { // > - end of inputsymbol
				sc.SetState(SCE_PL_DEFAULT);
			}
			break;
		...
		case SCE_PL_NUMBER:
			...
			if (sc.ch == '.') {
				...
				if (IsDecExponent(sc.ch)) {		// exponent, sign
				...
				if (IsHexExponent(sc.ch)) {	// hex float exponent, sign
					numState = PERLNUM_FLOAT_EXP;
					if (IsSign(sc.chNext)) {
						sc.Forward();
					}
			...
			if (HereDoc.State == 0) { // '<<' encountered
				...
				if (IsSpaceOrTab(delim_ch)) {
					...
					while ((i < endPos) && IsSpaceOrTab(delim_ch)) {
						delim_ch = static_cast<unsigned char>
										(styler.SafeGetCharAt(++i));
					}
					ws_skip = i - sc.currentPos - 1;
				}
				if (IsQuote(delim_ch) || delim_ch == '`') {
		...
		case SCE_PL_FORMAT: {
				...
				if (sc.Match('.')) {
					sc.Forward();
					if (sc.atLineEnd || IsCRLF(sc.ch, sc.chNext))
						sc.SetState(SCE_PL_DEFAULT);
				}
				while (!sc.atLineEnd)
					sc.Forward();
		...
		if (sc.state == SCE_PL_DEFAULT) {
			if (IsDigit(sc.ch) ||
				...
				if (sc.state == SCE_PL_WORD) {
					while (setWord.Contains(styler.SafeGetCharAt(fw)))
						fw++;
