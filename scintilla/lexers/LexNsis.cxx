							 ...
							 Accessor &styler, bool bElse, bool foldUtilityCmd)
{
	...
	if (s[0] == '!')
	{
		if (NsisCmp(s, "!ifndef", bIgnoreCase) == 0 ||
			NsisCmp(s, "!ifdef", bIgnoreCase) == 0 ||
			NsisCmp(s, "!ifmacrodef", bIgnoreCase) == 0 ||
			NsisCmp(s, "!ifmacrondef", bIgnoreCase) == 0 ||
			NsisCmp(s, "!if", bIgnoreCase) == 0 ||
			NsisCmp(s, "!macro", bIgnoreCase) == 0)
			newFoldlevel++;
		else if (NsisCmp(s, "!endif", bIgnoreCase) == 0 ||
				 NsisCmp(s, "!macroend", bIgnoreCase) == 0)
			newFoldlevel--;
	...
	else
	{
		if (NsisCmp(s, "Section", bIgnoreCase) == 0 ||
			NsisCmp(s, "SectionGroup", bIgnoreCase) == 0 ||
			NsisCmp(s, "Function", bIgnoreCase) == 0 ||
			NsisCmp(s, "SubSection", bIgnoreCase) == 0 ||
			NsisCmp(s, "PageEx", bIgnoreCase) == 0)
			newFoldlevel++;
		else if (NsisCmp(s, "SectionGroupEnd", bIgnoreCase) == 0 ||
				 NsisCmp(s, "SubSectionEnd", bIgnoreCase) == 0 ||
				 NsisCmp(s, "FunctionEnd", bIgnoreCase) == 0 ||
				 NsisCmp(s, "SectionEnd", bIgnoreCase) == 0 ||
...
static int classifyWordNsis(Sci_PositionU start, Sci_PositionU end,
...
static void ColouriseNsisDoc(Sci_PositionU startPos, Sci_Position length,
		...
		switch(state)
		{
			case SCE_NSIS_DEFAULT:
				...
				if (cCurrChar == '$' || isNsisChar(cCurrChar) || cCurrChar == '!')
				{
					...
					if (isNsisNumber(cCurrChar) && (IsSpaceOrTab(cNextChar) ||
													IsCRLF(cNextChar)))
...
static void FoldNsisDoc(Sci_PositionU startPos, Sci_Position length,
						int, WordList *[], Accessor &styler)
{
	...
	{
		if (styler.SafeGetCharAt(safeStartPos) == '/' &&
			styler.SafeGetCharAt(safeStartPos + 1) == '*')
			levelNext++;
		blockComment = true;
	}
