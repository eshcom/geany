	...
	Accessor &styler)
{
	...
	while (sc.More())
	{
		...
		if (sc.state == SCE_VHDL_OPERATOR) {
			...
			if (!isExtendedId && (!IsWordChar(sc.ch) || (sc.ch == '.'))) {
				char s[100];
		...
		} else if (sc.state == SCE_VHDL_COMMENT ||
				   sc.state == SCE_VHDL_COMMENTLINEBANG) {
			if (sc.atLineEnd) {
				sc.SetState(SCE_VHDL_DEFAULT);
			}
...
static bool IsCommentBlockStart(Sci_Position line, Accessor &styler)
{
	...
	for (Sci_Position i = pos; i < eol_pos; i++) {
		...
		if ((style == SCE_VHDL_BLOCK_COMMENT) &&
			(ch == '/') && (chNext == '*'))
			return true;
	}
	return false;
	...
	Accessor &styler)
{
	...
	for (j = startPos; j > 0; j--)
	{
		...
		{
			if (!IsWordChar(chPrev) && IsAlnumWordChar(ch) && (end != 0))
			{
				char s[32];
				Sci_PositionU k;
	...
	for (j = j + static_cast<Sci_PositionU>(strlen(prevWord));
		 j < endPos; j++)
	{
		char ch       = styler.SafeGetCharAt(j);
		int style     = styler.StyleAt(j);
	...
	for (Sci_PositionU i = startPos; i < endPos; i++)
	{
		...
		if (foldComment && atEOL)
		{
			...
			{
				if (IsCommentBlockStart(lineCurrent, styler) &&
					!IsCommentBlockEnd(lineCurrent, styler))
				{
					levelNext++;
				}
				else if (IsCommentBlockEnd(lineCurrent, styler) &&
						 !IsCommentBlockStart(lineCurrent, styler))
				{
					levelNext--;
				}
			}
		...
		if ((!IsCommentStyle(style)) && (style != SCE_VHDL_STRING))
		{
			...
			if (!IsWordChar(chPrev) && IsAlnumWordChar(ch))
			{
				lastStart = i;
			}
				...
				if (keywords.InList(s))
				{
							   ...
							   strcmp(s, "configuration") == 0)
					{
						...
						{ // check for instantiated unit by backward searching for the colon.
							...
							while (pos && (IsSpaceOrTab(chAtPos) || IsCRLF(chAtPos) ||
...
//=============================================================================
static void FoldVHDLDoc(Sci_PositionU startPos, Sci_Position length,
