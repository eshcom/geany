...
static bool HasPrevLineContent(StyleContext &sc) {
	...
	while ((--i + sc.currentPos) && !IsCRLF(sc.GetRelative(i)))
		;
	...
	while (sc.More()) {
		...
		// Headers
			...
			//SetStateAndZoom(SCE_TXT2TAGS_HEADER2, 2, '+', sc);
				...
				// Catch the special case of an unordered list
				if (sc.chNext == ' ' && IsSpaceOrTab(sc.GetRelative(1))) {
				 //    if (IsCRLF(sc.ch)) {
					 	//precharCount = 0;
				//		sc.SetState(SCE_TXT2TAGS_LINE_BEGIN);
						//sc.SetState(SCE_TXT2TAGS_PRECHAR);
		...
		else if (sc.state == SCE_TXT2TAGS_HEADER1 ||
				 sc.state == SCE_TXT2TAGS_HEADER2 ||
				 sc.state == SCE_TXT2TAGS_HEADER3 ||
				 sc.state == SCE_TXT2TAGS_HEADER4 ||
				 sc.state == SCE_TXT2TAGS_HEADER5 ||
				 sc.state == SCE_TXT2TAGS_HEADER6) {
			if (IsCRLF(sc.ch))
				sc.SetState(SCE_TXT2TAGS_LINE_BEGIN);
		}
		...
		if (sc.state == SCE_TXT2TAGS_PRECHAR) {
			...
			*/
			...
			// on a line by themselves
				...
				;
			// Unordered list
			else if ((sc.ch == '-') && IsSpaceOrTab(sc.chNext)) {
