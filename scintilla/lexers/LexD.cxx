...
static bool isWordStart(int ch) {
	return IsAlphaWordChar(ch) || !IsASCII(ch);
}

static bool isWord(int ch) {
	return IsAlnumWordChar(ch) || !IsASCII(ch);
}
...
void SCI_METHOD LexerD::Lex(Sci_PositionU startPos, Sci_Position length,
	...
	for (; sc.More(); sc.Forward()) {
		...
		switch (sc.state) {
			...
			case SCE_D_COMMENTDOCKEYWORD:
				if ((styleBeforeDCKeyword == SCE_D_COMMENTDOC) &&
						sc.Match('*', '/')) {
...
void SCI_METHOD LexerD::Fold(Sci_PositionU startPos, Sci_Position length,
							 int initStyle, IDocument *pAccess) {
	if (!options.fold)
		return;
	...
	int style = initStyle;
	bool foldAtElse = options.foldAtElseInt >= 0 ? options.foldAtElseInt != 0 :
												   options.foldAtElse;
