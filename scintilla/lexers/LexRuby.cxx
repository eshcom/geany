...
static inline bool isAlnumWordChar(char ch) {
...
static int ClassifyWordRb(Sci_PositionU start, Sci_PositionU end,
	...
	else if (keywords.InList(s) && ((start == 0) ||
			 !followsDot(start - 1, styler))) {
...
static bool isMatch(Accessor &styler, Sci_Position lengthDoc,
					Sci_Position pos, const char *val) {
...
static bool sureThisIsNotHeredoc(Sci_Position lt2StartPos,
	...
	Sci_Position exprStartPosn = findExpressionStart(lt2StartPos, lineStartPosn,
													 styler);
	// Find the first word after some whitespace
...
#define INNER_STRINGS_MAX_COUNT 5
	...
	for (i = startPos; i < lengthDoc; i++) {
		...
		if (state == SCE_RB_DEFAULT) {
			if (IsDigit(ch)) {
				...
				if ((i == 0 || IsCRLF(chPrev))
						&& chNext == 'b'
						&& styler.SafeGetCharAt(i + 2) == 'e'
						&& styler.SafeGetCharAt(i + 3) == 'g'
						&& styler.SafeGetCharAt(i + 4) == 'i'
						&& styler.SafeGetCharAt(i + 5) == 'n'
						&& !isAlnumWordChar(styler.SafeGetCharAt(i + 6))) {
					styler.ColourTo(i - 1, state);
					state = SCE_RB_POD;
				} else {
					styler.ColourTo(i - 1, state);
				...
				if (!(strchr("\"\'`_-", chNext2) || IsAlphaWordChar(chNext2))) {
				...
				if (chNext == ':') {
					...
					state = SCE_RB_DEFAULT;
					preferRE = false;
				} else if (isAlnumWordChar(chNext)) {
					state = SCE_RB_SYMBOL;
				} else if ((chNext == '@' || chNext == '$') &&
						   isAlnumWordChar(chNext2)) {
					// instance and global variable followed by an identifier
					advance_char(i, ch, chNext, chNext2);
					state = SCE_RB_SYMBOL;
				} else if (((chNext == '@' && chNext2 == '@')  ||
							(chNext == '$' && chNext2 == '-')) &&
						   isAlnumWordChar(styler.SafeGetCharAt(i+3))) {
				...
				if (strchr(q_chars, chNext) && !isAlnumWordChar(chNext2)) {
					Quote.New();
				...
				} else if (preferRE && !isAlnumWordChar(chNext)) {
					...
					advance_char(i, ch, chNext, chNext2); // pass by ref
					have_string = true;
				} else if (!isAlnumWordChar(chNext) && !IsWhiteSpace(chNext)) {
					// Ruby doesn't allow high bit chars here,
					// but the editor host might
					Quote.New();
					state = SCE_RB_STRING_QQ;
			...
			if (ch == '.' || !isAlnumWordChar(ch)) {
				...
				if (ch == '='
					&& isAlnumWordChar(chPrev)
					&& (chNext == '(' || IsWhiteSpace(chNext))
					&& (!strcmp(prevWord, "def") ||
					...
					int word_style = ClassifyWordRb(wordStartPos, i - 1, keywords,
													styler, prevWord);
					switch (word_style) {
					case SCE_RB_WORD:
			...
			if (HereDoc.State == 0) { // '<<' encountered
				...
				if (HereDoc.DelimiterLength >=
						static_cast<int>(sizeof(HereDoc.Delimiter)) - 1) {
					styler.ColourTo(i - 1, state);
					state = SCE_RB_ERROR;
					preferRE = false;
				}
			...
			if (IsWhiteSpace(ch) && i > 5 && IsCRLF(styler[i - 5])
	...
	if (state == SCE_RB_WORD) {
		...
		// classify it.
		(void) ClassifyWordRb(styler.GetStartSegment(), lengthDoc - 1,
							  keywords, styler, prevWord);
	} else {
...
static bool keywordIsModifier(const char *word,
	...
	while (lineStartPosn > 0) {
		...
		if (IsCRLF(ch)) {
			...
			} else if (chPrev2 == '\\' && IsCRLF(chPrev, ch)) {
				lineStartPosn = styler.LineStart(lineStart);
			} else {
				break;
			}
...
static bool keywordDoStartsLoop(Sci_Position pos, Accessor &styler) {
	...
	while (--pos >= lineStartPosn) {
		...
		if (style == SCE_RB_DEFAULT) {
			...
			for (start_word = pos;
					start_word >= lineStartPosn &&
					actual_style(styler.StyleAt(start_word)) == SCE_RB_WORD;
					start_word--) {
