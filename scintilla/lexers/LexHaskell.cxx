...
static int u_IsHaskellSymbol(int ch) {
	CharacterCategory c = CategoriseCharacter(ch);
	return c == ccPc || c == ccPd || c == ccPo || c == ccSm ||
		   c == ccSc || c == ccSk || c == ccSo;
}
...
static int HaskellIndentAmount(Accessor &styler, const Sci_Position line) {
	...
	if (styler.LineStart(line) == styler.Length() ||
		IsWhiteSpace(ch) || IsCommentStyle(style) ||
...
Sci_Position SCI_METHOD LexerHaskell::PropertySet(const char *key,
												  const char *val) {
...
void SCI_METHOD LexerHaskell::Lex(Sci_PositionU startPos, Sci_Position length,
								  int initStyle, IDocument *pAccess) {
	LexAccessor styler(pAccess);
	
	Sci_Position lineCurrent = styler.GetLine(startPos);
	
	HaskellLineInfo hs = HaskellLineInfo(lineCurrent ?
	...
	while (sc.More()) {
		...
		} else if (literate && hs.lmode == LITERATE_BLOCK &&
				   sc.atLineStart && sc.Match("\\end{code}")) {
			sc.SetState(SCE_HA_LITERATE_CODEDELIM);
		...
		// Haskell
			...
			if (sc.atLineEnd) {
				...
				sc.Forward();
			} else if (dot && IsDotDigit(sc.ch, sc.chNext, base)) {
				sc.Forward(2);
				dot = false;
			} else if (base == 10 && IsDecExponent(sc.ch) &&
					   (IsDigit(sc.chNext) || IsSign(sc.chNext))) {
				sc.Forward();
				if (IsSign(sc.ch))
					sc.Forward();
			} else {
...
void SCI_METHOD LexerHaskell::Fold(Sci_PositionU startPos, Sci_Position length,
								   int /*initStyle*/, IDocument *pAccess) {
	if (!options.fold)
		return;
	...
	while (lineCurrent <= docLines && lineCurrent <= maxLines) {
		...
		styler.SetLevel(lineCurrent, options.foldCompact ?
...
LexerModule lmHaskell(SCLEX_HASKELL, LexerHaskell::LexerFactoryHaskell,
					  "haskell", haskellWordListDesc);
LexerModule lmLiterateHaskell(SCLEX_LITERATEHASKELL,
