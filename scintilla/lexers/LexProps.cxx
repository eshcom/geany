...
static inline bool isOperator(int ch) {
	if (ch == '-')
		return false;
	return IsOperator(ch) || IsQuoteOrBackslash(ch)
			|| ch == '#' || ch == '$' || ch == '@';
}
	...
	for (; pos < endPos; pos++) {
		if (mailState == 0) {					// login
			if (styler[pos] == '@' && pos > 0 && pos < (endPos - 1)
				...
				mailState = 1;
				pos++;
			} else if (!isWordChar(styler[pos]) && styler[pos] != '.') {
				break;
			}
			...
			if (styler[pos] == '.') {
				...
				isDomainDot = true;
				isLastDot = true;
			} else if (isWordChar(styler[pos])) {
				isLastDot = false;
			} else if (IsWhiteSpace(styler[pos]) || pos == (endPos - 1)) {
...
static inline bool CheckSubVar(StyleContext &sc, Accessor &styler,
	...
	if ((sc.ch == '#' || sc.ch == '$' || sc.ch == '%')
		...
		for (Sci_PositionU i = sc.currentPos + 2; i < endPos; i++) {
			if (IsCRLF(styler[i])) { // end of line
				break;
			} else if (styler[i] == '}') {
...
static void ColourisePropsDoc(Sci_PositionU startPos, Sci_Position length,
	...
	for (; sc.More(); sc.Forward()) {
		...
		if (sc.atLineStart && stringCode == 0) {
			if (!allowInitialSpaces && IsSpaceOrTab(sc.ch)) {
		...
		switch (sc.state) {
			...
			case SCE_PROPS_HEXNUMBER:
				if (IsDigitOrDotDigit(sc.ch, sc.chNext, 16)) {
					if (sc.ch == '.') {
						if (sc.chPrev == '.') {
							sc.ChangeState(SCE_PROPS_VALUE);
							break;
		...
		if (sc.state == SCE_PROPS_OPER_VALUE ||
			...
			if (sc.chPrev != '.' && sc.Match('0', 'x')
				...
				continue;
					   ...
					   (IsDigitOrDotDigit(sc.ch, sc.chNext) ||
						(IsSign(sc.ch) &&
						 (sc.chNext == '.' ||
						  (IsDigit(sc.chNext) &&
				...
				for (Sci_PositionU i = sc.currentPos + 1; i < styler.Length(); i++) {
					if (IsCRLF(styler[i])) { // end of line
						...
						if (i == styler.Length() || IsCRLF(styler[i])) { // end of line
							if (strCode == -1) {
								strCode = 1; // multi-line string
								continue;
							} else if (strCode == 1) {
			...
			} else if (isWordChar(sc.ch) && /* exclude subtract-oper */
		...
		} else if (sc.state == SCE_PROPS_VALUE && !isWordChar(sc.ch)) {
			// sc.chPrev == word-char (alnum, '_', '-')
			// --END OF WORD--
			char word[100];
