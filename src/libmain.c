...
{
	{ "column", 0, 0, G_OPTION_ARG_INT, &cl_options.goto_column,
	  N_("Set initial column number to COLUMN for the first "
		 "opened file (useful in conjunction with --line)"), N_("COLUMN") },
	{ "config", 'c', 0, G_OPTION_ARG_FILENAME, &alternate_config,
	  N_("Use alternate configuration directory DIR"), N_("DIR") },
	{ "ft-names", 0, 0, G_OPTION_ARG_NONE, &ft_names,
	...
	{ "socket-file", 0, 0, G_OPTION_ARG_FILENAME, &cl_options.socket_filename,
	  N_("Use socket filename FILE for communication with "
		 "a running Geany instance"), N_("FILE") },
	{ "list-documents", 0, 0, G_OPTION_ARG_NONE, &cl_options.list_documents,
	  N_("Return a list of open documents in a running Geany instance"), NULL },
#endif
	{ "line", 'l', 0, G_OPTION_ARG_INT, &cl_options.goto_line,
	  N_("Set initial line number to LINE for the first opened file"), N_("LINE") },
	{ "no-msgwin", 'm', 0, G_OPTION_ARG_NONE, &no_msgwin,
	  N_("Don't show message window at startup"), NULL },
	{ "no-ctags", 'n', 0, G_OPTION_ARG_NONE, &ignore_global_tags,
...
#endif
	{ "print-prefix", 0, 0, G_OPTION_ARG_NONE, &print_prefix,
	  N_("Print Geany's installation prefix"), NULL },
	{ "read-only", 'r', 0, G_OPTION_ARG_NONE, &cl_options.readonly,
	  N_("Open all FILES in read-only mode (see documentation)"), NULL },
	{ "no-session", 's', G_OPTION_FLAG_REVERSE, G_OPTION_ARG_NONE, &cl_options.load_session,
	  N_("Don't load the previous session's files"), NULL },
#ifdef HAVE_VTE
	{ "no-terminal", 't', 0, G_OPTION_ARG_NONE, &no_vte,
...
/* special things for the initial setup of the checkboxes and
 * related stuff an action on a setting is only performed
 * if the setting is not equal to the program default
 * (all the following code is not perfect but it works for the moment) */
static void apply_settings(void)
{
	ui_update_fold_items();
	
	/* toolbar, message window and sidebar are by default visible,
	 * so don't change it if it is true */
	toolbar_show_hide();
...
static void on_window_active_changed(GtkWindow *window, GParamSpec *pspec,
									 gpointer data)
{
...
static void main_init(void)
{
	...
	g_signal_connect(main_widgets.window, "notify::is-active",
					 G_CALLBACK(on_window_active_changed), NULL);
	
	/* add recent projects to the Project menu */
	ui_widgets.recent_projects_menuitem = ui_lookup_widget(main_widgets.window,
														   "recent_projects1");
	...
	gtk_widget_set_name(ui_lookup_widget(main_widgets.window, "menubar1"),
						"GeanyMenubar");
...
gchar *main_get_argv_filename(const gchar *filename)
{
	...
	{
		/* use current dir */
		gchar *cur_dir = original_cwd == NULL ? g_get_current_dir()
...
static void get_line_and_column_from_filename(gchar *filename, gint *line, gint *column)
{
	...
	for (gsize i = strlen(filename) - 1; i >= 1; i--)
	{
...
static void change_working_directory_on_windows(void)
{
	...
	/* remember original working directory for use with
	 * opening files from the command line */
	original_cwd = g_get_current_dir();
	
	/* On Windows, change the working directory to the Geany installation path
	 * to not lock the directory of a file passed as command line argument
	 * (see bug #2626124).
 ...
 *
 *  @note Maybe you want to use the @link pluginsignals.c @c "geany-startup-complete"
 *  signal @endlink to get notified about the completed startup process.
 *
 ...
 *
 *  Note that this function only setups the gettext textdomain for you.
 *  You still have to adjust the build system of your plugin to get
 *  internationalisation support working properly.
 *
 ...
 *
 *  @param locale_dir The location where the translation files should be searched.
 *                    This is usually the @c LOCALEDIR macro, defined by the build system.
 *                    E.g. @c $prefix/share/locale.
 *                    Only used on non-Windows systems. On Windows, the directory
 *                    is determined by @c g_win32_get_package_installation_directory().
...
static void parse_command_line_options(gint *argc, gchar ***argv)
{
	...
	if (app->debug_mode)
	{	/* Since GLib 2.32 messages logged with levels INFO and DEBUG aren't output by
		 * the default log handler unless the G_MESSAGES_DEBUG environment variable
		 * contains the domain of the message or is set to the special value "all" */
...
static gint create_config_dir(void)
{
	...
	{
#ifndef G_OS_WIN32
		/* if we are *not* using an alternate config directory, we check
		 * whether the old one in ~/.geany still exists and try to move it */
		if (alternate_config == NULL)
		{
			...
			if (g_file_test(old_dir, G_FILE_TEST_EXISTS))
			{
				if (!dialogs_show_question_full(main_widgets.window,
										GTK_STOCK_YES, GTK_STOCK_QUIT,
										_("Move it now?"), "%s",
										_("Geany needs to move your old "
										  "configuration directory before starting.")))
					exit(0);
				...
				else
				{
					dialogs_show_msgbox(GTK_MESSAGE_WARNING,
							/* for translators: the third %s in brackets is the error
							 * message which describes why moving the dir didn't work */
							_("Your old configuration directory \"%s\" "
	...
	if (saved_errno == 0)
	{
		...
		if (saved_errno == 0 && !g_file_test(filedefs_readme, G_FILE_TEST_EXISTS))
		{
			gchar *text = g_strconcat(
							"Copy files from ", app->datadir,
							"/filedefs to this directory to overwrite them. To use "
							"the defaults, just delete the file in this directory.\n"
							"For more information read the documentation (in ",
							app->docdir,
							G_DIR_SEPARATOR_S "index.html or visit " GEANY_HOMEPAGE ").",
							NULL);
	...
	if (saved_errno == 0)
	{
		...
		{
			gchar *text = g_strconcat(
							"There are several template files in this directory. "
							"For these templates you can use wildcards.\n"
							"For more information read the documentation (in ",
							app->docdir,
							G_DIR_SEPARATOR_S "index.html or visit " GEANY_HOMEPAGE ").",
							NULL);
...
{
	gint mkdir_result = create_config_dir();
	
	if (mkdir_result != 0)
	{
		if (!dialogs_show_question(
					_("Configuration directory could not be created (%s).\n"
					  "There could be some problems using Geany without a "
...
{
	gchar *pfile = utils_get_path_from_uri(locale_filename);
	if (pfile != NULL)
	{
		if (use_session)
...
static void load_startup_files(gint argc, gchar **argv)
{
	...
	if (load_session)
	{
		/* load session files into tabs, as they are found
		 * in the session_files variable */
...
/* This prepends our own gtkrc file to the list of RC files
 * to be loaded by GTK at startup.
 * This function *has* to be called before gtk_init().
 *
 * We have a custom RC file defining various styles we need, and we want the user
 * to be able to override them (e.g. if they want -- or need -- other colors).
 * Fair enough, one would simply call gtk_rc_parse() with the appropriate filename.
 * However, the styling rules applies in the order they are loaded, then if we load
 * our styles after GTK has loaded the user's ones we'd override them.
 *
...
gint main_lib(gint argc, gchar **argv)
{
	...
	{
		const gchar *message = _("IPC socket could not be created, "
								 "see Help->Debug Messages for details.");
	...
	/* create the custom command menu after the keybindings have been
	 * loaded to have the proper accelerator shown for the menu items */
	...
	/* when we are really done with setting everything up and the main event loop
	 * is running, tell other components, mainly plugins, that startup is complete */
...
static gboolean do_main_quit(void)
{
	...
	if (ui_widgets.prefs_dialog && GTK_IS_WIDGET(ui_widgets.prefs_dialog))
		gtk_widget_destroy(ui_widgets.prefs_dialog);
	if (ui_widgets.open_fontsel && GTK_IS_WIDGET(ui_widgets.open_fontsel))
		gtk_widget_destroy(ui_widgets.open_fontsel);
	if (ui_widgets.open_colorsel && GTK_IS_WIDGET(ui_widgets.open_colorsel))
		gtk_widget_destroy(ui_widgets.open_colorsel);
 ...
 *  Currently the following files are reloaded: all template files,
 *  also new file templates and the 'New (with template)' menus will be updated,
 *  Snippets (snippets.conf), filetype extensions (filetype_extensions.conf),
 *  and 'settings' and 'build_settings' sections of the filetype definition files.
 *
 *  Plugins may call this function if they changed any of these files
 *  (e.g. a configuration file editor plugin).
 *
 *  @since 0.15
 **/
GEANY_API_SYMBOL
