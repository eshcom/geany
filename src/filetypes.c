...
/* Note: we use GeanyFiletypeID for some function arguments,
 * but GeanyFiletype is better; we should only use GeanyFiletype
 * for API functions. */
					...
					GeanyFiletypeGroupID group_id)
{
	...
	ft->title = filetype_make_title(title_name ? title_name : ft->name,
									title_type);
	ft->group = group_id;
}
...
#define FT_INIT(ft_id, parser_id, name, title_name, title_type, group_id)	\
	ft_init(GEANY_FILETYPES_##ft_id, TM_PARSER_##parser_id,					\
			name, title_name, TITLE_##title_type,							\
			GEANY_FILETYPE_GROUP_##group_id)
...
static gint cmp_filetype(gconstpointer pft1, gconstpointer pft2, gpointer data)
{
	...
	return by_name ? utils_str_casecmp(ft1->name, ft2->name)
...
const GSList *filetypes_get_sorted_by_name(void)
{
	...
	if (!list)
	{
		...
		list = g_slist_sort_with_data(list, cmp_filetype,
...
static void setup_config_file_menus(void)
{
	...
	SETPTR(f, g_build_filename(app->configdir, GEANY_FILEDEFS_SUBDIR,
...
static void create_set_filetype_menu(gboolean config)
{
	...
	foreach_slist(node, filetypes_by_title)
	{
		...
		GtkWidget *parent = (ft->group != GEANY_FILETYPE_GROUP_NONE) ?
											group_menus[ft->group] : menu;
...
static GeanyFiletype *detect_filetype_conf_file(const gchar *utf8_filename)
{
	...
	path = g_build_filename(app->configdir, GEANY_FILEDEFS_SUBDIR,
							"filetypes.", NULL);
	if (g_str_has_prefix(lfn, path))
		found = TRUE;
	else
	{
		SETPTR(path, g_build_filename(app->datadir, GEANY_FILEDEFS_SUBDIR,
									  "filetypes.", NULL));
		if (g_str_has_prefix(lfn, path))
			found = TRUE;
	}
...
/* This detects the filetype of the file pointed by 'utf8_filename'
 * and a list of filetype id's, terminated by -1.
 * The detected filetype of the file is checked against every id
 * in the passed list and if there is a match, TRUE is returned. */
...
static GeanyFiletype *find_shebang(const gchar *utf8_filename,
								   const gchar *line)
{
	GeanyFiletype *ft = NULL;
	...
	if (g_str_has_prefix(line, "<!DOCTYPE html") || g_str_has_prefix(line, "<html"))
	{
		 ...
		 * use the detected filetype, else assume HTML */
	...
	{
		/* HTML and DocBook files might also start with <?xml,
		 * so detect them based on filename extension and
		 * use the detected filetype, else assume XML */
		if (!shebang_find_and_match_filetype(utf8_filename,
...
/* Detect the filetype for the document,
...
static void load_indent_settings(GeanyFiletype *ft, GKeyFile *config,
								 GKeyFile *configh)
{
	ft->indent_width = utils_get_setting(integer, configh, config,
										 "indentation", "width", -1);
	ft->indent_type = utils_get_setting(integer, configh, config,
										"indentation", "type", -1);
	...
	switch (ft->indent_type)
	{
		...
		default:
			g_warning("Invalid indent type %d in file type %s",
					  ft->indent_type, ft->name);
			ft->indent_type = -1;
			break;
	}
}
...
static void load_settings(guint ft_id, GKeyFile *config, GKeyFile *configh)
{
	...
	/* default extension */
	result = utils_get_setting(string, configh, config,
	...
	filetypes[ft_id]->comment_use_indent =
		utils_get_setting(boolean, configh, config,
	...
	if (result != NULL)
	{
		...
		if (ft->lang == TM_PARSER_NONE)
			geany_debug("Cannot find tags parser '%s' for custom filetype '%s'.",
						result, ft->name);
		g_free(result);
	}
	...
	if (result != NULL)
	{
		...
		if (!ft->lexer_filetype)
			geany_debug("Cannot find lexer filetype '%s' for custom filetype '%s'.",
						result, ft->name);
		g_free(result);
	}
	
	ft->priv->symbol_list_sort_mode =
		utils_get_setting(integer, configh, config,
						  "settings", "symbol_list_sort_mode",
						  SYMBOLS_SORT_USE_PREVIOUS);
	ft->priv->xml_indent_tags =
		utils_get_setting(boolean, configh, config,
						  "settings", "xml_indent_tags", FALSE);
...
static void add_group_keys(GKeyFile *kf, const gchar *group, GeanyFiletype *ft)
{
	...
	if (!loaded)
		geany_debug("Could not read config file %s for [%s=%s]!",
					files[0], group, ft->name);
...
static void load_system_keyfile(GKeyFile *key_file, const gchar *file,
								GKeyFileFlags flags, GeanyFiletype *ft)
{
	GError *error = NULL;
...
static void read_extensions(GKeyFile *sysconfig, GKeyFile *userconfig)
{
	...
	for (i = 0; i < filetypes_array->len; i++)
	{
		...
		gchar **list = g_key_file_get_string_list(userset ? userconfig : sysconfig,
												  "Extensions", filetypes[i]->name,
												  &len, NULL);
...
static void read_group(GKeyFile *config, const gchar *group_name,
					   GeanyFiletypeGroupID group_id)
{
	gchar **names = g_key_file_get_string_list(config, "Groups", group_name,
											   NULL, NULL);
	gchar **name;
	...
	foreach_strv(name, names)
	{
		...
		if (ft)
		{
			ft->group = group_id;
			if (ft->priv->custom &&
				(group_id == GEANY_FILETYPE_GROUP_COMPILED ||
				 group_id == GEANY_FILETYPE_GROUP_SCRIPT))
			{
				SETPTR(ft->title, filetype_make_title(ft->name, TITLE_SOURCE_FILE));
			}
		}
		else
			geany_debug("Filetype '%s' not found for group '%s'!",
						*name, group_name);
	}
	g_strfreev(names);
}
...
static void read_filetype_config(void)
{
	gchar *sysconfigfile = g_build_filename(app->datadir,
											"filetype_extensions.conf", NULL);
	gchar *userconfigfile = g_build_filename(app->configdir,
											 "filetype_extensions.conf", NULL);
