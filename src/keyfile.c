...
/* some default settings which are used at the very first
 * start of Geany to fill the configuration file */
...
static void init_pref_groups(void)
{
	...
	stash_group_add_integer(group, &interface_prefs.tab_pos_sidebar,
	...
	stash_group_add_spin_button_integer(group, (gint *)&editor_prefs.autocompletion_update_freq,
										"autocompletion_update_freq", GEANY_MAX_SYMBOLS_UPDATE_FREQ,
										"spin_symbol_update_freq");
	stash_group_add_string(group, &editor_prefs.color_scheme, "color_scheme", NULL);
	...
	stash_group_add_spin_button_integer(group, (gint *)&file_prefs.mru_length,
										"mru_length", GEANY_DEFAULT_MRU_LENGTH,
										"spin_mru");
...
static void save_recent_files(GKeyFile *config, GQueue *queue, gchar const *key)
{
	...
	for (guint i = 0; i < file_prefs.mru_length; i++)
	{
	...
	g_key_file_set_string_list(config, "files", key, (const gchar **)recent_files,
...
static gchar *get_session_file_string(GeanyDocument *doc)
{
	...
	gchar *escaped_filename = g_uri_escape_string(locale_filename, NULL, TRUE);
	
	gchar *fname = g_strdup_printf("%d;%s;%d;E%s;%d;%d;%d;%s;%d;%d",
...
{
	gint npage = gtk_notebook_get_current_page(GTK_NOTEBOOK(main_widgets.notebook));
	...
	guint max = gtk_notebook_get_n_pages(GTK_NOTEBOOK(main_widgets.notebook));
	guint j = 0;
	gchar entry[16];
	...
	for (guint i = 0; i < max; i++)
	{
		...
		if (doc && doc->real_path)
		{
			...
			gchar *fname = get_session_file_string(doc);
	...
	if (vte_info.have_vte)
	{
		...
		// esh: fixed overwriting VTE/last_dir of the current project from
		//      the previous project in case there was an incomplete command
		//      in the terminal in the previous project
...
static void save_dialog_prefs(GKeyFile *config)
{
	...
	/* general */
	g_key_file_set_boolean(config, PACKAGE, "pref_main_load_session",
						   prefs.load_session);
	g_key_file_set_boolean(config, PACKAGE, "pref_main_project_session",
						   project_prefs.project_session);
	g_key_file_set_boolean(config, PACKAGE, "pref_main_project_file_in_basedir",
						   project_prefs.project_file_in_basedir);
	g_key_file_set_boolean(config, PACKAGE, "pref_main_confirm_project_closure",
						   project_prefs.confirm_project_closure);
	g_key_file_set_boolean(config, PACKAGE, "pref_main_save_winpos",
						   prefs.save_winpos);
	g_key_file_set_boolean(config, PACKAGE, "pref_main_save_wingeom",
						   prefs.save_wingeom);
	g_key_file_set_boolean(config, PACKAGE, "pref_main_confirm_exit",
						   prefs.confirm_exit);
	g_key_file_set_boolean(config, PACKAGE, "pref_main_suppress_status_messages",
						   prefs.suppress_status_messages);
	g_key_file_set_boolean(config, PACKAGE, "switch_msgwin_pages",
						   prefs.switch_to_status);
	g_key_file_set_boolean(config, PACKAGE, "beep_on_errors",
						   prefs.beep_on_errors);
	g_key_file_set_boolean(config, PACKAGE, "auto_focus",
						   prefs.auto_focus);
	g_key_file_set_boolean(config, PACKAGE, "force_focus_to_new_doc",
						   prefs.force_focus_to_new_doc);
	g_key_file_set_boolean(config, PACKAGE, "set_caret_to_start_of_line",
						   prefs.set_caret_to_start_of_line);
	
	/* interface */
	g_key_file_set_boolean(config, PACKAGE, "sidebar_symbols_visible",
						   interface_prefs.sidebar_symbols_visible);
	g_key_file_set_boolean(config, PACKAGE, "sidebar_openfiles_visible",
	...
	g_key_file_set_boolean(config, PACKAGE, "show_notebook_tabs",
						   interface_prefs.show_notebook_tabs);
	g_key_file_set_boolean(config, PACKAGE, "show_tab_cross",
						   file_prefs.show_tab_cross);
	g_key_file_set_boolean(config, PACKAGE, "tab_order_ltr",
						   file_prefs.tab_order_ltr);
	g_key_file_set_boolean(config, PACKAGE, "tab_order_beside",
						   file_prefs.tab_order_beside);
	g_key_file_set_integer(config, PACKAGE, "tab_pos_editor",
						   interface_prefs.tab_pos_editor);
	g_key_file_set_integer(config, PACKAGE, "tab_pos_msgwin",
						   interface_prefs.tab_pos_msgwin);
	g_key_file_set_boolean(config, PACKAGE, "use_native_windows_dialogs",
						   interface_prefs.use_native_windows_dialogs);
	g_key_file_set_boolean(config, PACKAGE, "switch_to_msgwin_messages",
						   interface_prefs.switch_to_msgwin_messages);
	
	/* display */
	g_key_file_set_boolean(config, PACKAGE, "show_indent_guide",
						   editor_prefs.show_indent_guide);
	g_key_file_set_boolean(config, PACKAGE, "show_white_space",
						   editor_prefs.show_white_space);
	g_key_file_set_boolean(config, PACKAGE, "show_line_endings",
						   editor_prefs.show_line_endings);
	g_key_file_set_boolean(config, PACKAGE, "show_markers_margin",
						   editor_prefs.show_markers_margin);
	g_key_file_set_boolean(config, PACKAGE, "show_linenumber_margin",
						   editor_prefs.show_linenumber_margin);
	g_key_file_set_boolean(config, PACKAGE, "long_line_enabled",
						   editor_prefs.long_line_enabled);
	g_key_file_set_integer(config, PACKAGE, "long_line_type",
						   editor_prefs.long_line_type);
	g_key_file_set_integer(config, PACKAGE, "long_line_column",
						   editor_prefs.long_line_column);
	g_key_file_set_string(config, PACKAGE, "long_line_color",
						  editor_prefs.long_line_color);
	
	/* editor */
	g_key_file_set_integer(config, PACKAGE, "symbolcompletion_max_height",
						   editor_prefs.symbolcompletion_max_height);
	g_key_file_set_integer(config, PACKAGE, "symbolcompletion_min_chars",
						   editor_prefs.symbolcompletion_min_chars);
	g_key_file_set_boolean(config, PACKAGE, "use_folding", editor_prefs.folding);
	
	g_key_file_set_boolean(config, PACKAGE, "unfold_all_children",
						   editor_prefs.unfold_all_children);
	g_key_file_set_boolean(config, PACKAGE, "use_indicators",
						   editor_prefs.use_indicators);
	g_key_file_set_boolean(config, PACKAGE, "line_wrapping",
						   editor_prefs.line_wrapping);
	g_key_file_set_boolean(config, PACKAGE, "auto_close_xml_tags",
						   editor_prefs.auto_close_xml_tags);
	g_key_file_set_boolean(config, PACKAGE, "complete_snippets",
						   editor_prefs.complete_snippets);
	g_key_file_set_boolean(config, PACKAGE, "auto_complete_symbols",
						   editor_prefs.auto_complete_symbols);
	g_key_file_set_boolean(config, PACKAGE, "pref_editor_disable_dnd",
						   editor_prefs.disable_dnd);
	g_key_file_set_boolean(config, PACKAGE, "pref_editor_smart_home_key",
						   editor_prefs.smart_home_key);
	g_key_file_set_boolean(config, PACKAGE, "pref_editor_newline_strip",
						   editor_prefs.newline_strip);
	g_key_file_set_integer(config, PACKAGE, "line_break_column",
						   editor_prefs.line_break_column);
	g_key_file_set_boolean(config, PACKAGE, "auto_continue_multiline",
						   editor_prefs.auto_continue_multiline);
	g_key_file_set_string(config, PACKAGE, "comment_toggle_mark",
						  editor_prefs.comment_toggle_mark);
	g_key_file_set_boolean(config, PACKAGE, "scroll_stop_at_last_line",
						   editor_prefs.scroll_stop_at_last_line);
	g_key_file_set_integer(config, PACKAGE, "autoclose_chars",
						   editor_prefs.autoclose_chars);
	
	/* files */
	g_key_file_set_string(config, PACKAGE, "pref_editor_default_new_encoding",
						  encodings[file_prefs.default_new_encoding].charset);
	g_key_file_set_string(config, PACKAGE, "pref_editor_default_open_encoding",
						  file_prefs.default_open_encoding >= 0
							? encodings[file_prefs.default_open_encoding].charset
							: "none");
	g_key_file_set_integer(config, PACKAGE, "default_eol_character",
						   file_prefs.default_eol_character);
	g_key_file_set_boolean(config, PACKAGE, "pref_editor_new_line",
						   file_prefs.final_new_line);
	g_key_file_set_boolean(config, PACKAGE, "pref_editor_ensure_convert_line_endings",
						   file_prefs.ensure_convert_new_lines);
	g_key_file_set_boolean(config, PACKAGE, "pref_editor_replace_tabs",
						   file_prefs.replace_tabs);
	g_key_file_set_boolean(config, PACKAGE, "pref_editor_trail_space",
						   file_prefs.strip_trailing_spaces);
	
	/* toolbar */
	g_key_file_set_boolean(config, PACKAGE, "pref_toolbar_show",
						   toolbar_prefs.visible);
	g_key_file_set_boolean(config, PACKAGE, "pref_toolbar_append_to_menu",
						   toolbar_prefs.append_to_menu);
	g_key_file_set_boolean(config, PACKAGE, "pref_toolbar_use_gtk_default_style",
						   toolbar_prefs.use_gtk_default_style);
	g_key_file_set_boolean(config, PACKAGE, "pref_toolbar_use_gtk_default_icon",
						   toolbar_prefs.use_gtk_default_icon);
	g_key_file_set_integer(config, PACKAGE, "pref_toolbar_icon_style",
						   toolbar_prefs.icon_style);
	g_key_file_set_integer(config, PACKAGE, "pref_toolbar_icon_size",
						   toolbar_prefs.icon_size);
	
	/* templates */
	g_key_file_set_string(config, PACKAGE, "pref_template_developer",
						  template_prefs.developer);
	g_key_file_set_string(config, PACKAGE, "pref_template_company",
						  template_prefs.company);
	g_key_file_set_string(config, PACKAGE, "pref_template_mail",
						  template_prefs.mail);
	g_key_file_set_string(config, PACKAGE, "pref_template_initial",
						  template_prefs.initials);
	g_key_file_set_string(config, PACKAGE, "pref_template_version",
						  template_prefs.version);
	g_key_file_set_string(config, PACKAGE, "pref_template_year",
						  template_prefs.year_format);
	g_key_file_set_string(config, PACKAGE, "pref_template_date",
						  template_prefs.date_format);
	g_key_file_set_string(config, PACKAGE, "pref_template_datetime",
						  template_prefs.datetime_format);
	
	/* tools settings */
	g_key_file_set_string(config, "tools", "terminal_cmd",
						  tool_prefs.term_cmd ? tool_prefs.term_cmd : "");
	g_key_file_set_string(config, "tools", "browser_cmd",
						  tool_prefs.browser_cmd ? tool_prefs.browser_cmd : "");
	g_key_file_set_string(config, "tools", "grep_cmd",
						  tool_prefs.grep_cmd ? tool_prefs.grep_cmd : "");
	g_key_file_set_string(config, PACKAGE, "context_action_cmd",
	...
	g_key_file_set_string(config, "printing", "print_cmd",
						  printing_prefs.external_print_cmd
							? printing_prefs.external_print_cmd : "");
	g_key_file_set_boolean(config, "printing", "use_gtk_printing",
						   printing_prefs.use_gtk_printing);
	g_key_file_set_boolean(config, "printing", "print_line_numbers",
						   printing_prefs.print_line_numbers);
	g_key_file_set_boolean(config, "printing", "print_page_numbers",
						   printing_prefs.print_page_numbers);
	g_key_file_set_boolean(config, "printing", "print_page_header",
						   printing_prefs.print_page_header);
	g_key_file_set_boolean(config, "printing", "page_header_basename",
						   printing_prefs.page_header_basename);
	g_key_file_set_string(config, "printing", "page_header_datefmt",
						  printing_prefs.page_header_datefmt);
	
	/* gui */
	g_key_file_set_string(config, "gui", "compiler_error_color",
						  gui_prefs.compiler_error_color);
	g_key_file_set_string(config, "gui", "compiler_context_color",
						  gui_prefs.compiler_context_color);
	g_key_file_set_string(config, "gui", "compiler_message_color",
						  gui_prefs.compiler_message_color);
	g_key_file_set_string(config, "gui", "msgwin_selected_color",
						  gui_prefs.msgwin_selected_color);
	g_key_file_set_string(config, "gui", "msgwin_filepath_color",
						  gui_prefs.msgwin_filepath_color);
	g_key_file_set_string(config, "gui", "msgwin_operator_color",
						  gui_prefs.msgwin_operator_color);
	g_key_file_set_string(config, "gui", "msgwin_number_color",
						  gui_prefs.msgwin_number_color);
	g_key_file_set_string(config, "gui", "sidebar_parent_color",
						  gui_prefs.sidebar_parent_color);
	...
	if (vte_info.have_vte)
	{
		...
		g_key_file_set_boolean(config, "VTE", "ignore_menu_bar_accel",
...
{
	g_key_file_set_boolean(config, PACKAGE, "sidebar_visible",
						   ui_prefs.sidebar_visible);
	g_key_file_set_boolean(config, PACKAGE, "statusbar_visible",
						   interface_prefs.statusbar_visible);
	g_key_file_set_boolean(config, PACKAGE, "msgwindow_visible",
	...
	if (prefs.save_winpos || prefs.save_wingeom)
	{
		...
		gtk_window_get_position(GTK_WINDOW(main_widgets.window),
								&ui_prefs.geometry[0], &ui_prefs.geometry[1]);
		gtk_window_get_size(GTK_WINDOW(main_widgets.window),
							&ui_prefs.geometry[2], &ui_prefs.geometry[3]);
		
		GdkWindowState wstate = gdk_window_get_state(gtk_widget_get_window(
														main_widgets.window));
	...
	g_key_file_set_string(config, PACKAGE, "custom_date_format",
						  ui_prefs.custom_date_format);
	if (ui_prefs.custom_commands)
	{
		g_key_file_set_string_list(config, PACKAGE, "custom_commands",
								   (const gchar **)ui_prefs.custom_commands,
								   g_strv_length(ui_prefs.custom_commands));
		g_key_file_set_string_list(config, PACKAGE, "custom_commands_labels",
								   (const gchar **)ui_prefs.custom_commands_labels,
								   g_strv_length(ui_prefs.custom_commands_labels));
	}
}
...
{
	gsize len = 0;
	gchar **recent_files = g_key_file_get_string_list(config, "files", key, &len,
													  NULL);
	if (recent_files)
	{
		for (gsize i = 0; i < len && i < file_prefs.mru_length; i++)
		{
...
/*
 * Load session list from the given keyfile, and store it in
 * the global session_files variable for later file loading
 * */
void configuration_load_session_files(GKeyFile *config, gboolean read_recent_files)
{
	...
	session_notebook_page = utils_get_setting_integer(config, "files",
													  "current_page", -1);
...
static void load_dialog_prefs(GKeyFile *config)
{
	...
	{
		gboolean suppress_search_dialogs = utils_get_setting_boolean(config, PACKAGE,
												"pref_main_suppress_search_dialogs",
												FALSE);
		
		if (!g_key_file_has_key(config, "search", "pref_search_always_wrap", NULL))
			g_key_file_set_boolean(config, "search", "pref_search_always_wrap",
								   suppress_search_dialogs);
		
		if (!g_key_file_has_key(config, "search", "pref_search_hide_find_dialog", NULL))
			g_key_file_set_boolean(config, "search", "pref_search_hide_find_dialog",
								   suppress_search_dialogs);
	}
	
	/* general */
	prefs.confirm_exit = utils_get_setting_boolean(config, PACKAGE,
												"pref_main_confirm_exit", FALSE);
	prefs.suppress_status_messages = utils_get_setting_boolean(config, PACKAGE,
												"pref_main_suppress_status_messages", FALSE);
	prefs.load_session = utils_get_setting_boolean(config, PACKAGE,
												"pref_main_load_session", TRUE);
	project_prefs.project_session = utils_get_setting_boolean(config, PACKAGE,
												"pref_main_project_session", TRUE);
	project_prefs.project_file_in_basedir = utils_get_setting_boolean(config, PACKAGE,
												"pref_main_project_file_in_basedir", FALSE);
	project_prefs.confirm_project_closure = utils_get_setting_boolean(config, PACKAGE,
												"pref_main_confirm_project_closure", FALSE);
	prefs.save_winpos = utils_get_setting_boolean(config, PACKAGE,
												"pref_main_save_winpos", TRUE);
	prefs.save_wingeom = utils_get_setting_boolean(config, PACKAGE,
												"pref_main_save_wingeom", prefs.save_winpos);
	prefs.beep_on_errors = utils_get_setting_boolean(config, PACKAGE,
												"beep_on_errors", TRUE);
	prefs.switch_to_status = utils_get_setting_boolean(config, PACKAGE,
												"switch_msgwin_pages", FALSE);
	prefs.auto_focus = utils_get_setting_boolean(config, PACKAGE, "auto_focus", FALSE);
	
	prefs.force_focus_to_new_doc = utils_get_setting_boolean(config, PACKAGE,
												"force_focus_to_new_doc", FALSE);
	prefs.set_caret_to_start_of_line = utils_get_setting_boolean(config, PACKAGE,
												"set_caret_to_start_of_line", FALSE);
	
	/* interface */
	interface_prefs.tab_pos_editor = utils_get_setting_integer(config, PACKAGE,
												"tab_pos_editor", GTK_POS_TOP);
	interface_prefs.tab_pos_msgwin = utils_get_setting_integer(config, PACKAGE,
												"tab_pos_msgwin",GTK_POS_LEFT);
	interface_prefs.sidebar_symbols_visible =
								utils_get_setting_boolean(config, PACKAGE,
												"sidebar_symbols_visible", TRUE);
	interface_prefs.sidebar_openfiles_visible =
								utils_get_setting_boolean(config, PACKAGE,
												"sidebar_openfiles_visible", TRUE);
	interface_prefs.statusbar_visible = utils_get_setting_boolean(config, PACKAGE,
												"statusbar_visible", TRUE);
	file_prefs.tab_order_ltr = utils_get_setting_boolean(config, PACKAGE,
												"tab_order_ltr", TRUE);
	file_prefs.tab_order_beside = utils_get_setting_boolean(config, PACKAGE,
												"tab_order_beside", FALSE);
	file_prefs.show_tab_cross = utils_get_setting_boolean(config, PACKAGE,
												"show_tab_cross", TRUE);
	interface_prefs.show_notebook_tabs = utils_get_setting_boolean(config, PACKAGE,
												"show_notebook_tabs", TRUE);
	interface_prefs.editor_font = utils_get_setting_string(config, PACKAGE,
												"editor_font", GEANY_DEFAULT_FONT_EDITOR);
	interface_prefs.tagbar_font = utils_get_setting_string(config, PACKAGE,
	...
	interface_prefs.switch_to_msgwin_messages =
								utils_get_setting_boolean(config, PACKAGE,
												"switch_to_msgwin_messages", FALSE);
	/* display, editor */
	editor_prefs.long_line_enabled = utils_get_setting_boolean(config, PACKAGE,
												"long_line_enabled", TRUE);
	editor_prefs.long_line_type = utils_get_setting_integer(config, PACKAGE,
												"long_line_type", 0);
	...
	editor_prefs.long_line_color = utils_get_setting_string(config, PACKAGE,
												"long_line_color", "#C2EBC2");
	editor_prefs.long_line_column = utils_get_setting_integer(config, PACKAGE,
												"long_line_column", 72);
	editor_prefs.symbolcompletion_min_chars = utils_get_setting_integer(config, PACKAGE,
												"symbolcompletion_min_chars",
												GEANY_MIN_SYMBOLLIST_CHARS);
	editor_prefs.symbolcompletion_max_height = utils_get_setting_integer(config, PACKAGE,
												"symbolcompletion_max_height",
												GEANY_MAX_SYMBOLLIST_HEIGHT);
	editor_prefs.line_wrapping = utils_get_setting_boolean(config, PACKAGE,
												"line_wrapping", FALSE); // default is off for better performance
	editor_prefs.use_indicators = utils_get_setting_boolean(config, PACKAGE,
												"use_indicators", TRUE);
	editor_prefs.show_indent_guide = utils_get_setting_boolean(config, PACKAGE,
												"show_indent_guide", FALSE);
	editor_prefs.show_white_space = utils_get_setting_boolean(config, PACKAGE,
												"show_white_space", FALSE);
	editor_prefs.show_line_endings = utils_get_setting_boolean(config, PACKAGE,
												"show_line_endings", FALSE);
	editor_prefs.scroll_stop_at_last_line = utils_get_setting_boolean(config, PACKAGE,
												"scroll_stop_at_last_line", TRUE);
	editor_prefs.auto_close_xml_tags = utils_get_setting_boolean(config, PACKAGE,
												"auto_close_xml_tags", TRUE);
	editor_prefs.complete_snippets = utils_get_setting_boolean(config, PACKAGE,
												"complete_snippets", TRUE);
	editor_prefs.auto_complete_symbols = utils_get_setting_boolean(config, PACKAGE,
												"auto_complete_symbols", TRUE);
	editor_prefs.folding = utils_get_setting_boolean(config, PACKAGE,
												"use_folding", TRUE);
	editor_prefs.unfold_all_children = utils_get_setting_boolean(config, PACKAGE,
												"unfold_all_children", FALSE);
	editor_prefs.show_markers_margin = utils_get_setting_boolean(config, PACKAGE,
												"show_markers_margin", TRUE);
	editor_prefs.show_linenumber_margin = utils_get_setting_boolean(config, PACKAGE,
												"show_linenumber_margin", TRUE);
	editor_prefs.disable_dnd = utils_get_setting_boolean(config, PACKAGE,
												"pref_editor_disable_dnd", FALSE);
	editor_prefs.smart_home_key = utils_get_setting_boolean(config, PACKAGE,
												"pref_editor_smart_home_key", TRUE);
	editor_prefs.newline_strip = utils_get_setting_boolean(config, PACKAGE,
												"pref_editor_newline_strip", FALSE);
	editor_prefs.line_break_column = utils_get_setting_integer(config, PACKAGE,
												"line_break_column", 72);
	editor_prefs.auto_continue_multiline = utils_get_setting_boolean(config, PACKAGE,
												"auto_continue_multiline", TRUE);
	editor_prefs.comment_toggle_mark = utils_get_setting_string(config, PACKAGE,
												"comment_toggle_mark", GEANY_TOGGLE_MARK);
	editor_prefs.autoclose_chars = utils_get_setting_integer(config, PACKAGE,
												"autoclose_chars", 0);
	...
	tmp_string = utils_get_setting_string(config, PACKAGE,
										  "pref_editor_default_new_encoding",
										  default_charset);
	if (tmp_string)
	{
		const GeanyEncoding *enc = encodings_get_from_charset(tmp_string);
		file_prefs.default_new_encoding = enc ? enc->idx : GEANY_ENCODING_UTF_8;
		g_free(tmp_string);
	}
	...
	if (tmp_string)
	{
		...
		file_prefs.default_open_encoding = enc ? enc->idx : -1;
		g_free(tmp_string);
	}
	
	file_prefs.default_eol_character = utils_get_setting_integer(config, PACKAGE,
												"default_eol_character", 
												GEANY_DEFAULT_EOL_CHARACTER);
	file_prefs.replace_tabs = utils_get_setting_boolean(config, PACKAGE,
												"pref_editor_replace_tabs", FALSE);
	file_prefs.ensure_convert_new_lines = utils_get_setting_boolean(config, PACKAGE,
												"pref_editor_ensure_convert_line_endings",
												FALSE);
	file_prefs.final_new_line = utils_get_setting_boolean(config, PACKAGE,
												"pref_editor_new_line", TRUE);
	file_prefs.strip_trailing_spaces = utils_get_setting_boolean(config, PACKAGE,
												"pref_editor_trail_space", FALSE);
	
	/* toolbar */
	toolbar_prefs.visible = utils_get_setting_boolean(config, PACKAGE,
												"pref_toolbar_show", TRUE);
	toolbar_prefs.append_to_menu = utils_get_setting_boolean(config, PACKAGE,
												"pref_toolbar_append_to_menu", FALSE);
	{
		toolbar_prefs.use_gtk_default_style = utils_get_setting_boolean(config, PACKAGE,
												"pref_toolbar_use_gtk_default_style",
												TRUE);
		if (!toolbar_prefs.use_gtk_default_style)
			toolbar_prefs.icon_style = utils_get_setting_integer(config, PACKAGE,
												"pref_toolbar_icon_style",
												GTK_TOOLBAR_ICONS);
		toolbar_prefs.use_gtk_default_icon = utils_get_setting_boolean(config, PACKAGE,
												"pref_toolbar_use_gtk_default_icon",
												TRUE);
		if (!toolbar_prefs.use_gtk_default_icon)
			toolbar_prefs.icon_size = utils_get_setting_integer(config, PACKAGE,
												"pref_toolbar_icon_size",
	...
	if (vte_info.load_vte && vte_info.load_vte_cmdline /* not disabled on the cmdline */)
	{
		...
		const gchar *shell = pw ? pw->pw_shell : "/bin/sh";
		
#ifdef __APPLE__
		/* Geany is started using launchd on OS X and we don't get any
		 * environment variables so PS1 isn't defined.
		 * Start as a login shell to read the corresponding config files. */
		...
		if (EMPTY(vte_info.dir) && pw) // last dir is not set - fallback to user's home directory
			SETPTR(vte_info.dir, g_strdup(pw->pw_dir));
		else if (!vte_info.dir && !pw) // fallback to root
		...
		vc->ignore_menu_bar_accel = utils_get_setting_boolean(config, "VTE",
		...
		/* various VTE prefs.
		 * this can't be done in init_pref_groups() because we need to know
		 * the value of vte_info.load_vte, and `vc` to be initialized */
		StashGroup *group = stash_group_new("VTE");
		configuration_add_various_pref_group(group, "terminal");
		
		stash_group_add_string(group, &vc->send_cmd_prefix, "send_cmd_prefix", "");
		stash_group_add_boolean(group, &vc->send_selection_unsafe,
								"send_selection_unsafe", FALSE);
	}
#endif
	/* templates */
	template_prefs.developer = utils_get_setting_string(config, PACKAGE,
										"pref_template_developer", g_get_real_name());
	template_prefs.company = utils_get_setting_string(config, PACKAGE,
										"pref_template_company", "");
	template_prefs.version = utils_get_setting_string(config, PACKAGE,
										"pref_template_version", "1.0");
	
	tmp_string = utils_get_initials(template_prefs.developer);
	template_prefs.initials = utils_get_setting_string(config, PACKAGE,
	...
	template_prefs.year_format = utils_get_setting_string(config, PACKAGE,
										"pref_template_year", GEANY_TEMPLATES_FORMAT_YEAR);
	template_prefs.date_format = utils_get_setting_string(config, PACKAGE,
										"pref_template_date", GEANY_TEMPLATES_FORMAT_DATE);
	template_prefs.datetime_format = utils_get_setting_string(config, PACKAGE,
										"pref_template_datetime", GEANY_TEMPLATES_FORMAT_DATETIME);
	
	/* tools */
	gchar *cmd = utils_get_setting_string(config, "tools", "terminal_cmd", "");
	if (EMPTY(cmd))
	{
		...
		if (!EMPTY(cmd))
		{
			tmp_string = cmd;
#ifdef G_OS_WIN32
			cmd = strstr(cmd, "cmd.exe") ? g_strconcat(cmd, " /Q /C %c", NULL)
										 : g_strconcat(cmd, " %c", NULL);
#else
	...
	tool_prefs.browser_cmd = utils_get_setting_string(config, "tools", "browser_cmd",
													  GEANY_DEFAULT_TOOLS_BROWSER);
	tool_prefs.grep_cmd = utils_get_setting_string(config, "tools", "grep_cmd",
												   GEANY_DEFAULT_TOOLS_GREP);
	tool_prefs.context_action_cmd = utils_get_setting_string(config, PACKAGE,
															 "context_action_cmd", "");
	
	/* printing */
	gchar *tmp_string2 = g_find_program_in_path(GEANY_DEFAULT_TOOLS_PRINTCMD);
	...
	printing_prefs.external_print_cmd = utils_get_setting_string(config, "printing",
													"print_cmd", tmp_string);
	g_free(tmp_string);
	g_free(tmp_string2);
	
	printing_prefs.use_gtk_printing = utils_get_setting_boolean(config, "printing",
													"use_gtk_printing", TRUE);
	printing_prefs.print_line_numbers = utils_get_setting_boolean(config, "printing",
													"print_line_numbers", TRUE);
	printing_prefs.print_page_numbers = utils_get_setting_boolean(config, "printing",
													"print_page_numbers", TRUE);
	printing_prefs.print_page_header = utils_get_setting_boolean(config, "printing",
													"print_page_header", TRUE);
	printing_prefs.page_header_basename = utils_get_setting_boolean(config, "printing",
													"page_header_basename", FALSE);
	printing_prefs.page_header_datefmt = utils_get_setting_string(config, "printing",
													"page_header_datefmt", "%c");
	
	/* gui */
	gui_prefs.compiler_error_color = utils_get_setting_string(config, "gui",
													"compiler_error_color", "#ff0000");
	gui_prefs.compiler_context_color = utils_get_setting_string(config, "gui",
													"compiler_context_color", "#7f0000");
	gui_prefs.compiler_message_color = utils_get_setting_string(config, "gui",
													"compiler_message_color", "#0000D0");
	gui_prefs.msgwin_selected_color = utils_get_setting_string(config, "gui",
													"msgwin_selected_color", "#19A680");
	gui_prefs.msgwin_filepath_color = utils_get_setting_string(config, "gui",
													"msgwin_filepath_color", "#716094");
	gui_prefs.msgwin_operator_color = utils_get_setting_string(config, "gui",
													"msgwin_operator_color", "#5F6782");
	gui_prefs.msgwin_number_color = utils_get_setting_string(config, "gui",
													"msgwin_number_color", "#A65300");
	gui_prefs.sidebar_parent_color = utils_get_setting_string(config, "gui",
													"sidebar_parent_color", "#665f38");
...
{
	ui_prefs.sidebar_visible = utils_get_setting_boolean(config, PACKAGE,
													"sidebar_visible", TRUE);
	ui_prefs.msgwindow_visible = utils_get_setting_boolean(config, PACKAGE,
													"msgwindow_visible", TRUE);
	ui_prefs.fullscreen = utils_get_setting_boolean(config, PACKAGE,
													"fullscreen", FALSE);
	ui_prefs.custom_date_format = utils_get_setting_string(config, PACKAGE,
													"custom_date_format", "");
	ui_prefs.custom_commands = g_key_file_get_string_list(config, PACKAGE,
													"custom_commands", NULL, NULL);
	ui_prefs.custom_commands_labels = g_key_file_get_string_list(config, PACKAGE,
	...
	{
		guint cc_len = ui_prefs.custom_commands
									? g_strv_length(ui_prefs.custom_commands) : 0;
		guint cc_labels_len = ui_prefs.custom_commands_labels
									? g_strv_length(ui_prefs.custom_commands_labels) : 0;
		
		if (cc_labels_len < cc_len)			// not enough items, resize and fill
		{
			ui_prefs.custom_commands_labels =
					g_realloc(ui_prefs.custom_commands_labels,
							  (cc_len + 1) * sizeof *ui_prefs.custom_commands_labels);
			for (guint i = cc_labels_len; i < cc_len; i++)
				ui_prefs.custom_commands_labels[i] = g_strdup("");
			ui_prefs.custom_commands_labels[cc_len] = NULL;
		}
		else if (cc_labels_len > cc_len)	// too many items, cut off
		{
			for (guint i = cc_len; i < cc_labels_len; i++)
			{
	...
	gsize geo_len;
	gint *geo = g_key_file_get_integer_list(config, PACKAGE, "geometry", &geo_len,
											NULL);
	if (!geo || geo_len < 5)
	{
	...
	{	/* don't use insane values but when main windows was maximized last time,
		 * pos might be negative (due to differences in root window and window decorations) */
	...
	vpan_position = utils_get_setting_integer(config, PACKAGE, "msgwindow_position",
						geo ? (GEANY_MSGWIN_HEIGHT + geo[3] - 440)
...
gboolean configuration_load(void)
{
	...
	{	/* config file does not (yet) exist, so try to load a global
		 * config file which may be created by distributors */
	...
	/* this signal can be used e.g. to delay building UI elements
	 * until settings have been read */
...
static gboolean open_session_file(gchar **tmp, guint len)
{
	...
	guint pos = atoi(tmp[0]);
	const gchar *ft_name = tmp[1];
	gboolean ro = atoi(tmp[2]);
	
	const gchar *encoding = isdigit(tmp[3][0])
								? encodings_get_charset_from_index(atoi(tmp[3]))
								: &(tmp[3][1]);
	gint indent_type = atoi(tmp[4]);
	gboolean auto_indent = atoi(tmp[5]);
	gboolean line_wrapping = atoi(tmp[6]);
	/* try to get the locale equivalent for the filename */
	gchar *unescaped_filename = g_uri_unescape_string(tmp[7], NULL);
	...
	if (g_file_test(locale_filename, G_FILE_TEST_IS_REGULAR))
	{
		...
		GeanyDocument *doc = document_open_file_full(NULL, locale_filename,
													 pos, ro, ft, encoding);
		if (doc)
		{
			gint indent_width = doc->editor->indent_width;
			if (len > 9) indent_width = atoi(tmp[9]);
...
/* Open session files
 * Note: notebook page switch handler and adding to recent files list
 * is always disabled for all files opened within this function */
void configuration_open_files(void)
{
	...
	{
		/* explicitly trigger a notebook page switch after unsetting
		 * main_status.opening_session_files for callbacks to run
		 * (and update window title, encoding settings, and so on) */
		...
		/* if target page is current page, switch to another page
		 * first to really trigger an event */
