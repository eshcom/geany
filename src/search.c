...
static void on_find_dialog_response(GtkDialog *dialog, gint response,
									gpointer user_data);
...
static void init_prefs(void)
{
	...
	stash_group_add_boolean(group, &find_dlg.all_expanded,
		"find_all_expanded", FALSE);
	stash_group_add_boolean(group, &replace_dlg.all_expanded,
...
static GtkWidget *add_find_checkboxes(GtkDialog *dialog)
{
	...
	g_signal_connect(check_regexp, "toggled",
					 G_CALLBACK(on_find_replace_checkbutton_toggled),
					 dialog);
	...
	{
		GtkWidget *check_back = gtk_check_button_new_with_mnemonic(
													_("Search _backwards"));
	...
	g_signal_connect(checkbox2, "toggled",
					 G_CALLBACK(on_widget_toggled_set_insensitive),
					 checkbox5);
...
{
	gtk_dialog_response(GTK_DIALOG(find_dlg.dialog),
...
static void create_find_dialog(void)
{
	...
	g_signal_connect(button, "clicked",
					 G_CALLBACK(send_find_dialog_response),
	...
	ui_hbutton_box_copy_layout(GTK_BUTTON_BOX(gtk_dialog_get_action_area(
												GTK_DIALOG(find_dlg.dialog))),
							   GTK_BUTTON_BOX(bbox));
...
{
	gtk_dialog_response(GTK_DIALOG(replace_dlg.dialog),
						GPOINTER_TO_INT(user_data));
}
...
static void create_replace_dialog(void)
{
	...
	gtk_button_set_image(GTK_BUTTON(button),
						 gtk_image_new_from_stock(GTK_STOCK_FIND_AND_REPLACE,
	...
	g_signal_connect(replace_dlg.find_entry, "key-press-event",
					 G_CALLBACK(on_widget_key_pressed_set_focus),
	...
	gtk_container_add(GTK_CONTAINER(bbox), button);
	g_signal_connect(button, "clicked",
					 G_CALLBACK(send_replace_dialog_response),
...
static void update_fif_file_mode_combo(void)
{
	GtkTreeModel *model = gtk_combo_box_get_model(
							GTK_COMBO_BOX(fif_dlg.files_mode_combo));
	GtkTreeIter iter;
...
static void create_fif_dialog(void)
{
	...
	gtk_dialog_add_button(GTK_DIALOG(fif_dlg.dialog), GTK_STOCK_FIND,
						  GTK_RESPONSE_ACCEPT);
...
void search_show_find_in_files_dialog_full(const gchar *text, const gchar *dir)
{
	...
	/* bring the dialog back in the foreground in case
	 * it is already open but the focus is away */
						  ...
						  struct Sci_TextToFind *ttf)
{
	...
	while (search_find_text(sci, flags, ttf, &info) != -1)
	{
		 ...
		 * note we cannot assume a match will always be empty or not and then break out,
		 * since matches like "a?(?=b)" will sometimes be empty and sometimes not */
...
gint search_mark_all(GeanyDocument *doc, const gchar *search_text,
...
static void on_find_entry_activate_backward(GtkEntry *entry, gpointer user_data)
{
	...
	if (search_data.flags & GEANY_FIND_REGEXP)
		...
		on_find_dialog_response(NULL, GEANY_RESPONSE_FIND_PREVIOUS_BY_ENTER,
								user_data);
}


static GeanyFindFlags int_search_flags(gint match_case, gint whole_word,
									   gint regexp, gint multiline,
									   gint word_start)
{
									...
									gpointer user_data)
{
	...
	else
	{
		...
		g_free(search_data.text);
		g_free(search_data.original_text);
		search_data.text = g_strdup(gtk_entry_get_text(
								GTK_ENTRY(gtk_bin_get_child(GTK_BIN(user_data)))));
		search_data.original_text = g_strdup(search_data.text);
		search_data.flags = int_search_flags(settings.find_case_sensitive,
											 settings.find_match_whole_word,
											 settings.find_regexp,
											 settings.find_regexp_multiline,
		...
		ui_combo_box_add_to_history(GTK_COMBO_BOX_TEXT(user_data),
									search_data.original_text, 0);
		...
		switch (response)
		{
			...
			{
				gint result = document_find_text(doc, search_data.text,
								search_data.original_text, search_data.flags,
			...
			case GEANY_RESPONSE_FIND_IN_FILE:
				search_find_usage(search_data.text, search_data.original_text,
								  search_data.flags, FALSE);
				break;
			
			case GEANY_RESPONSE_FIND_IN_SESSION:
				search_find_usage(search_data.text, search_data.original_text,
								  search_data.flags, TRUE);
				break;
...
static void on_replace_entry_activate(GtkEntry *entry, gpointer user_data)
{
	on_replace_dialog_response(NULL, search_prefs.replace_and_find_by_default
										? GEANY_RESPONSE_REPLACE_AND_FIND
										: GEANY_RESPONSE_REPLACE, NULL);
}


static void replace_in_session(GeanyDocument *doc, GeanyFindFlags search_flags_re,
							   gboolean search_replace_escape_re, const gchar *find,
									   ...
									   gpointer user_data)
{
	...
	ui_combo_box_add_to_history(GTK_COMBO_BOX_TEXT(replace_dlg.find_combobox),
								original_find, 0);
	...
	switch (response)
	{
		...
		{
			gint rep = document_replace_text(doc, find, original_find, replace,
											 search_flags_re, search_backwards_re);
			if (rep != -1)
				document_find_text(doc, find, original_find, search_flags_re,
								   search_backwards_re, NULL, TRUE, NULL);
			break;
		}
		case GEANY_RESPONSE_REPLACE:
			document_replace_text(doc, find, original_find, replace,
								  search_flags_re, search_backwards_re);
			break;
		
		case GEANY_RESPONSE_FIND:
		{
			gint result = document_find_text(doc, find, original_find,
											 search_flags_re, search_backwards_re,
											 NULL, TRUE, GTK_WIDGET(dialog));
			ui_set_search_entry_background(replace_dlg.find_entry, (result > -1));
			break;
		}
		case GEANY_RESPONSE_REPLACE_IN_FILE:
			if (!document_replace_all(doc, find, replace, original_find,
									  original_replace, search_flags_re))
				utils_beep();
			break;
		
		case GEANY_RESPONSE_REPLACE_IN_SESSION:
			replace_in_session(doc, search_flags_re, search_replace_escape_re,
							   find, replace, original_find, original_replace);
			break;
		
		case GEANY_RESPONSE_REPLACE_IN_SEL:
			document_replace_sel(doc, find, replace, original_find,
								 original_replace, search_flags_re);
			break;
	}
	switch (response)
...
{
	gtk_window_get_position(GTK_WINDOW(fif_dlg.dialog),
							&fif_dlg.position[0],
							&fif_dlg.position[1]);
	...
	if (response == GTK_RESPONSE_ACCEPT)
	{
		...
		GeanyEncodingIndex enc_idx = ui_encodings_combo_box_get_active_encoding(
										GTK_COMBO_BOX(fif_dlg.encoding_combo));
		...
		else if (!EMPTY(search_text))
		{
			...
			{
				ui_combo_box_add_to_history(GTK_COMBO_BOX_TEXT(search_combo),
											search_text, 0);
				ui_combo_box_add_to_history(GTK_COMBO_BOX_TEXT(fif_dlg.files_combo),
											NULL, 0);
				ui_combo_box_add_to_history(GTK_COMBO_BOX_TEXT(dir_combo),
											utf8_dir, 0);
									 ...
									 const gchar *enc)
{
	...
	else
	{
		ui_set_statusbar(TRUE, _("Cannot execute grep tool \"%s\": %s. "
						 ...
						 tool_prefs.grep_cmd, error->message);
		g_error_free(error);
	}
...
static void search_finished(GPid child_pid, gint status, gpointer user_data)
{
	...
	switch (exit_status)
	{
		case 0:
		{
			...
			gchar *text = ngettext("Search completed with %d match.",
...
static gchar *get_regex_match_string(const gchar *text, const GeanyMatchInfo *match,
									 guint nth)
{
...
static gint find_regex(ScintillaObject *sci, guint pos, GRegex *regex,
					   gboolean multiline, GeanyMatchInfo *match)
{
	const gchar *text;
	GMatchInfo *minfo;
	...
	if (multiline)
	{
		/* Warning: any SCI calls will invalidate 'text'
		 * after calling SCI_GETCHARACTERPOINTER */
	...
	else /* single-line mode, manually match against each line */
	{
		...
		for (;;)
		{
			...
			if (g_regex_match_full(regex, text, end - start, pos - start,
								   0, &minfo, NULL))
			{
				offset = start;
				break;
...
{
	g_warn_if_fail(!(flags & GEANY_FIND_REGEXP) ||
				   !(flags & GEANY_FIND_MULTILINE));
						  ...
						  const gchar *replace_text)
{
	...
	while (str->str[i])
	{
		...
		grp = get_regex_match_string(match->match_text - match->matches[0].start,
									 match, c - '0');
					  ...
					  struct Sci_TextToFind *ttf, GeanyMatchInfo **match_)
{
	...
	if (~flags & GEANY_FIND_REGEXP)
	{
		...
		if (ret != -1 && match_)
			*match_ = match_info_new(flags, ttf->chrgText.cpMin,
									 ttf->chrgText.cpMax);
		return ret;
	}
...
void search_find_again(gboolean change_direction)
{
	...
	if (search_data.text)
	{
		...
		gint result = document_find_text(doc, search_data.text,
										 search_data.original_text,
										 search_data.flags,
										 change_direction ? forward : !forward,
										 NULL, FALSE, NULL);
		if (result > -1)
			editor_display_current_line(doc->editor, 0.3F);
