...
/* Frees the workspace structure and all child source files.
 * Use only when exiting from the main program.
*/
...
{
	GPtrArray *new_tags = tm_tags_merge(*big_array, small_array,
										workspace_tags_sort_attrs, FALSE);
...
gboolean tm_workspace_load_global_tags(const char *tags_file, TMParserType mode)
{
	...
	new_tags = tm_tags_merge(theWorkspace->global_tags, file_tags,
							 global_tags_sort_attrs, TRUE);
				   ...
				   TMSourceFile *file, TMParserType lang, gboolean namespace)
{
	...
	for (i = 0; i < 5; i++)
	{
		...
		for (j = 0; j < type_tags->len; j++)
		{
			...
			if (tm_tag_is_anon(test_tag) && (file != test_tag->file ||
											 test_tag->file == NULL))
				continue;
					   ...
					   TMParserType lang, gboolean member, const gchar *current_scope)
{
	...
	for (i = 0; i < tags->len && !member_tags; i++)
	{
		...
		{
			if (tag->type & tm_tag_typedef_t)
				member_tags = find_scope_members(searched_array, tag->name,
												 tag->file, lang, TRUE);
			else
				member_tags = find_scope_members_tags(tag->file ? tag->file->tags_array
																: searched_array,
													  tag, TRUE);
		}
		else if (tag->var_type)  /* variable: scope search */
		{
				...
				member_at_method_scope(tags, current_scope, tag, lang))
			{
				...
				member_tags = find_scope_members(searched_array, tag_type,
												 tag->file, lang, FALSE);
				g_free(tag_type);
			}
		}
