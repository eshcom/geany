...
static void prepare_taglist(GtkWidget *tree, GtkTreeStore *store)
{
	...
	/* callback for changed selection not necessary,
...
static void create_default_tag_tree(void)
{
	...
	gtk_viewport_set_shadow_type(GTK_VIEWPORT(tv.default_tag_tree),
								 GTK_SHADOW_NONE);
	
	GtkWidget *label = gtk_label_new(_("No symbols found"));
...
void sidebar_update_tag_list(GeanyDocument *doc, gboolean update)
{
	...
	if (gtk_notebook_get_current_page(GTK_NOTEBOOK(main_widgets.sidebar_notebook))
															!= TREEVIEW_SYMBOL)
	...
	{	/* updating the tag list in the left tag window */
		if (doc->priv->tag_tree == NULL)
		{
			doc->priv->tag_store = gtk_tree_store_new(
									SYMBOLS_N_COLUMNS, GDK_TYPE_PIXBUF,
									G_TYPE_STRING, TM_TYPE_TAG, G_TYPE_STRING);
...
static void prepare_openfiles(void)
{
	...
	/* tooltips */
	ui_tree_view_set_tooltip_text_column(GTK_TREE_VIEW(tv.tree_openfiles),
										 DOCUMENTS_FILENAME);
	/* selection handling */
...
static gboolean find_tree_iter_dir(GtkTreeIter *iter, const gchar *dir)
{
	...
	gchar *name;
	gtk_tree_model_get(GTK_TREE_MODEL(store_openfiles), iter,
					   DOCUMENTS_SHORTNAME, &name, -1);
	
	gboolean result = utils_filenamecmp(name, dir) == 0;
	g_free(name);
...
void sidebar_openfiles_add(GeanyDocument *doc)
{
	...
	/* check if new parent */
	if (parent && gtk_tree_model_iter_n_children(GTK_TREE_MODEL(store_openfiles),
												 parent) == 1)
	{
		GtkTreePath *path;
...
static void create_openfiles_popup_menu(void)
{
	...
	g_signal_connect(item, "activate",
					 G_CALLBACK(on_openfiles_document_action),
					 GINT_TO_POINTER(OPENFILES_ACTION_COPY_NAME));
...
static void unfold_parent(GtkTreeIter *iter)
{
	...
	if (gtk_tree_model_iter_parent(GTK_TREE_MODEL(store_openfiles),
								   &parent, iter))
	{
		GtkTreePath *path = gtk_tree_model_get_path(GTK_TREE_MODEL(store_openfiles),
													&parent);
		gtk_tree_view_expand_row(GTK_TREE_VIEW(tv.tree_openfiles), path, TRUE);
		gtk_tree_path_free(path);
	}
...
{
	gtk_tree_model_foreach(GTK_TREE_MODEL(store_openfiles),
						   tree_model_find_node, doc);
}


/* callbacks */
static void copy_name_to_clipboard(GeanyDocument *doc, gboolean parentitem,
								   gboolean fullpath)
{
...
static gboolean openfiles_go_to_selection(GtkTreeSelection *selection,
										  guint keyval)
{
	/* use switch_notebook_page to ignore changing the notebook page
	...
	if (gtk_tree_selection_get_selected(selection, &model, &iter))
	{
		...
		if (line > 0)
		{
			...
			if (doc != NULL)
			{
				...
				if (keyval != GDK_space && !(state & GEANY_CTRL_MOD_MASK))
									 ...
									 gpointer user_data)
{
	...
	if (ui_is_keyval_enter_or_return(event->keyval) || event->keyval == GDK_space)
	{
		 ...
		 * doing so will prevent further handlers to be run in most cases,
		 * but the only one is our own, so guess it's fine. */
										...
										G_GNUC_UNUSED gpointer user_data)
{
	...
	{
		if (widget == tv.tree_openfiles)
			on_openfiles_document_action(NULL,
					GINT_TO_POINTER(OPENFILES_ACTION_REMOVE));
	}
...
static void on_sidebar_switch_page(GtkNotebook *notebook, gpointer page,
...
void sidebar_init(void)
{
	...
	stash_group_add_widget_property(group, &ui_prefs.sidebar_page,
