...
static gboolean encodings_charset_equals(const gchar *a, const gchar *b)
{
	...
	while (*a && *b)
	{
		...
		{
			/* either there was a real separator, or we need a implicit one
			 * (a chage from alpha to numeric or so) */
...
{
	if (!charset) return GEANY_ENCODING_UTF_8;
...
{
	if (!charset) return &encodings[GEANY_ENCODING_UTF_8];
...
{
	const GeanyEncoding *encoding = encodings_get_from_charset(charset);
 ...
 * regex_compile() is used to compile regular expressions on program init
 * and keep it in memory for faster access when opening a file.
 * Pre-compiled regexps will be freed on program exit.
 */
										   ...
										   gpointer data)
{
	...
	gint cmp_res = strcmp(a_string, b_string);
	g_free(a_string);
	g_free(b_string);
	return cmp_res;
}
...
GtkTreeStore *encodings_encoding_store_new(gboolean has_detect)
{
	...
	if (has_detect)
	{
		...
		gtk_tree_store_set(store, &iter_current, 0, GEANY_ENCODINGS_MAX,
	...
	for (gint i = 0; i < GEANY_ENCODINGS_MAX; i++)
	{
		...
		gchar *encoding_string = encodings_to_string(&encodings[i]);
	...
	gtk_tree_sortable_set_sort_func(GTK_TREE_SORTABLE(store), 1,
									encoding_combo_store_sort_func, NULL, NULL);
	return store;
}
											  ...
											  const gchar *charset, gboolean fast)
{
	...
	gchar *converted_contents = g_convert(buffer, size, "UTF-8", charset, NULL,
										  &bytes_written, &conv_error);
	if (fast)
	{
		utf8_content = converted_contents;
		if (conv_error) g_error_free(conv_error);
	}
	else if (conv_error || !g_utf8_validate(converted_contents, bytes_written, NULL))
	{
		if (conv_error)
		{
			geany_debug("Couldn't convert from %s to UTF-8 (%s).",
						charset, conv_error->message);
...
{
	for (guint i = 0; i < G_N_ELEMENTS(pregs); i++)
	{
		gchar *charset;
														...
														gchar **used_encoding)
{
	...
	gboolean check_locale = !g_get_charset(&locale_charset);
	
	/* First check for preferred charset, if specified */
	gint preferred_charset = file_prefs.default_open_encoding;
	
	if (preferred_charset == (gint)encodings[GEANY_ENCODING_NONE].idx
		|| preferred_charset < 0 || preferred_charset >= GEANY_ENCODINGS_MAX)
		preferred_charset = -1;
	
	/* -1 means "Preferred charset" */
	for (gint i = -1; i < GEANY_ENCODINGS_MAX; i++)
	{
		...
		geany_debug("Trying to convert %" G_GSIZE_FORMAT
					" bytes of data from %s into UTF-8.", size, charset);
		
		utf8_content = encodings_convert_to_utf8_from_charset(buffer, size,
															  charset, FALSE);
...
/**
 *  Tries to convert @a buffer into UTF-8 encoding and store the detected
 *  original encoding in @a used_encoding.
 *
 *  @param buffer the input string to convert.
 *  @param size the length of the string, or -1 if the string is nul-terminated.
 *  @param used_encoding @out @optional return location of the detected encoding
 *         of the input string, or @c NULL.
 *
 *  @return @nullable If the conversion was successful, a newly allocated
 *          nul-terminated string, which must be freed with @c g_free().
 *          Otherwise @c NULL.
 **/
GEANY_API_SYMBOL
								 ...
								 gchar **used_encoding)
{
	...
	gchar *regex_charset = encodings_check_regexes(buffer, size);
	gchar *utf8 = encodings_convert_to_utf8_with_suggestion(buffer, size,
															regex_charset,
															used_encoding);
											  ...
											  guint *bom_len)
{
	...
	if (len >= 4)
	{
		...
		if ((string[0] == 0x2b && string[1] == 0x2f && string[2] == 0x76)
			&& (string[3] == 0x38 || string[3] == 0x39 ||
				string[3] == 0x2b || string[3] == 0x2f))
...
{
	if (string && (strncmp(string, "UTF", 3) == 0 ||
				   strncmp(string, "UCS", 3) == 0))
		return TRUE;
	return FALSE;
}
...
static gboolean handle_forced_encoding(BufferData *buffer, const gchar *forced_enc)
{
	...
	GeanyEncodingIndex enc_idx = encodings_scan_unicode_bom(buffer->data,
															buffer->size, NULL);
...
static gboolean handle_encoding(BufferData *buffer, GeanyEncodingIndex enc_idx)
{
	...
	else
	{
		...
		if (buffer->enc == NULL) /* either there was no BOM or the BOM encoding failed */
		{
			...
			{
				/* detect the encoding */
				gchar *converted_text = encodings_convert_to_utf8_with_suggestion(
													buffer->data, buffer->size,
													regex_charset, &buffer->enc);
				if (converted_text == NULL)
				{
					g_free(regex_charset);
					return FALSE;
...
{
	/* temporarily retrieve the encoding idx based on the BOM
	 * to suppress the following warning if we have a BOM */
	GeanyEncodingIndex tmp_enc_idx = encodings_scan_unicode_bom(buffer->data,
																buffer->size, NULL);
	
	/* check whether the size of the loaded data is equal to the size of the file in the
	 * filesystem file size may be 0 to allow opening files in /proc/ which have typically
	...
	if (buffer->bom) handle_bom(buffer);
	return TRUE;
}
										...
										gboolean *has_bom, gboolean *partial)
{
	...
	*size = buffer.len;
	...
	if (has_bom) *has_bom = buffer.bom;
	if (partial) *partial = buffer.partial;
