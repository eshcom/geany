...
#define USE_GIO_FILE_OPERATIONS (!file_prefs.use_safe_file_saving && \
								 file_prefs.use_gio_unsafe_file_saving)
...
static void document_undo_add_internal(GeanyDocument *doc, guint type,
									   gpointer data);
...
static GtkWidget *document_show_message(GeanyDocument *doc, GtkMessageType msgtype,
										void (*response_cb)(GtkWidget *info_bar,
															gint response_id,
															GeanyDocument *doc),
...
GeanyDocument *document_get_from_notebook_child(GtkWidget *page)
{
	...
	ScintillaObject *sci = locate_sci_in_container(page);
 ...
 *
 *  @return @transfer{none} @nullable The corresponding document for the
 * 									  given notebook page, or @c NULL.
 **/
GEANY_API_SYMBOL
GeanyDocument *document_get_from_page(guint page_num)
{
	...
	GtkWidget *parent = gtk_notebook_get_nth_page(
 ...
 *
 *  @return @transfer{none} @nullable A pointer to the current document or
 * 									  @c NULL if there are no opened documents.
 **/
GEANY_API_SYMBOL
GeanyDocument *document_get_current(void)
{
	gint cur_page = gtk_notebook_get_current_page(
						GTK_NOTEBOOK(main_widgets.notebook));
	
	return (cur_page == -1) ? NULL : document_get_from_page((guint)cur_page);
}
 ...
 *  The result is also truncated to a maximum of @a length characters
 *  in case the filename is very long.
 *
 *  @param doc The document to use.
 *  @param length The length of the resulting string or -1 to use a default value.
 *
 *  @return The ellipsized last part of the filename of @a doc,
...
gchar *document_get_basename_for_display(GeanyDocument *doc, gint length)
{
	...
	gchar *short_name = utils_str_middle_truncate(base_name, (guint)length);
	g_free(base_name);
									 ...
									 gboolean fullpath)
{
	...
	gchar *path = parentitem ? g_path_get_dirname(DOC_FILENAME(doc))
							 : g_strdup(DOC_FILENAME(doc));
	
	gchar *name = fullpath ? g_strdup(path) : g_path_get_basename(path);
	g_free(path);
...
void document_update_tab_label(GeanyDocument *doc)
{
	...
	gchar *short_name = document_get_basename_for_display(doc, -1);
	
	/* we need to use the event box for the tooltip, 
	 * labels don't get the necessary events */
...
/**
 *  Updates the tab labels, the status bar, the window title and
 *  some save-sensitive buttons according to the document's save state.
...
{
	if (doc->priv->colourise_needed) return;
	 ...
	 * This ensures we don't start colourising before all documents
	 * are opened/saved, only once the editor is drawn. */
	gtk_widget_queue_draw(GTK_WIDGET(doc->editor->sci));
}


#ifdef USE_GIO_FILEMON
static void monitor_file_changed_cb(G_GNUC_UNUSED GFileMonitor *monitor,
...
static void monitor_file_setup(GeanyDocument *doc)
{
	...
	/* Disable file monitoring completely for remote files (i.e. remote GIO files) as
	 * GFileMonitor doesn't work at all for remote files and legacy polling is too slow. */
	if (!doc->priv->is_remote)
	{
		...
		if (locale_filename != NULL && g_file_test(locale_filename,
												   G_FILE_TEST_EXISTS))
		{
			/* get a file monitor and connect to the 'changed' signal */
			GFile *file = g_file_new_for_path(locale_filename);
			doc->priv->monitor = g_file_monitor_file(file, G_FILE_MONITOR_NONE,
													 NULL, NULL);
			g_signal_connect(doc->priv->monitor, "changed",
							 G_CALLBACK(monitor_file_changed_cb), doc);
			
			/* we set the rate limit according to the GUI pref but
			 * it's most probably not used */
			g_file_monitor_set_rate_limit(doc->priv->monitor,
										  file_prefs.disk_check_timeout * 1000);
			g_object_unref(file);
		}
...
/**
 *  Removes the given notebook tab at @a page_num and clears
 *  all related information in the document list.
 *
 ...
 *
 *  @param utf8_filename @nullable The file name in UTF-8 encoding,
 *                                 or @c NULL to open a file as "untitled".
 *  @param ft @nullable The filetype to set or @c NULL to detect it
 *                      from @a filename if not @c NULL.
 *  @param text @nullable The initial content of the file
 *                        (in UTF-8 encoding), or @c NULL.
 *
								 ...
								 const gchar *text)
{
	...
	{
		gchar *tmp = utils_strdupa(utf8_filename); /* work around const */
		utils_tidy_path(tmp);
		utf8_filename = tmp;
	}
	...
	if (text)
	{
		...
		utils_ensure_same_eol_characters(template,
	...
	/* guess the filetype from the filename if one is given */
	if (ft == NULL && utf8_filename != NULL)
	...
	g_signal_connect(doc->editor->sci, "sci-notify",
					 G_CALLBACK(editor_sci_notify_cb), doc->editor);
 ...
 *  @param ft @nullable The filetype for the document or @c NULL
 *                      to auto-detect the filetype.
 *  @param forced_enc @nullable The file encoding to use or @c NULL
...
{
	return document_open_file_full(NULL, locale_filename, 0,
								   readonly, ft, forced_enc);
}
...
/* loads textfile data, verifies and converts to forced_enc or UTF-8.
 * Also handles BOM. */
							   ...
							   const gchar *forced_enc)
{
	...
	if (USE_GIO_FILE_OPERATIONS)
	{
		...
		g_file_load_contents(file, NULL, &filedata->data,
							 &filedata->len, NULL, &err);
		g_object_unref(file);
	}
	else
		g_file_get_contents(locale_filename, &filedata->data,
							&filedata->len, &err);
	if (err)
	{
	...
	if (!encodings_convert_to_utf8_auto(&filedata->data, &filedata->len,
										forced_enc, &filedata->enc,
										&filedata->bom, &filedata->readonly))
	{
		if (forced_enc)
			ui_set_statusbar(TRUE, _("The file \"%s\" is not valid %s."),
							 display_filename, forced_enc);
		else
			ui_set_statusbar(TRUE, _("The file \"%s\" does not look like a text file "
...
/* Sets the cursor position on opening a file. First it sets the line
 * when cl_options.goto_line is set, otherwise it sets the line when pos
 * is greater than zero and finally it sets the column
...
static gint set_cursor_position(GeanyEditor *editor, gint pos, gboolean reload)
{
	...
	{	/* goto column which was specified on command line
		 * and then undefine the column */
		gint new_pos = sci_get_current_position(editor->sci) +
...
gboolean document_detect_indent_type(GeanyDocument *doc, GeanyIndentType *type_)
{
	...
	if (spaces == 0 && tabs == 0) return FALSE;
									   ...
									   GeanyFiletype *ft, const gchar *forced_enc)
{
	...
	{	/* doc possibly changed */
		...
		else
		{
			...
			undo_reload_data->actions_count =
						g_trash_stack_height(&doc->priv->undo_actions);
		...
		doc->priv->mtime = filedata.mtime; /* get the modification time
											  from file and keep it */
		...
		if (!reload)
		{
			/* "the" SCI signal (connect after initial setup(i.e. adding text)) */
			g_signal_connect(doc->editor->sci, "sci-notify",
							 G_CALLBACK(editor_sci_notify_cb),
							 doc->editor);
		...
		if (reload)
			editor_set_indent(doc->editor, doc->editor->indent_type,
							  doc->editor->indent_width); /* resetup sci */
		else
		...
		else
		{
			...
			/* For translators: this is the status window message for opening a file.
			 * %d is the number of the newly opened file,
			 * %s indicates whether the file is opened read-only
			 * (it is replaced with the string ", read-only"). */
			msgwin_status_add(_("File %s opened (%d%s)."),
							  display_filename,
							  gtk_notebook_get_n_pages(GTK_NOTEBOOK(main_widgets.notebook)),
	...
	/* finally, let the editor widget grab the focus so
...
void document_open_file_list(const gchar *data, gsize length)
{
	...
	gchar **items = g_strsplit(data,
							   utils_get_eol_char(utils_get_line_endings(data, length)),
							   0);
 ...
 *
 *  @param filenames @elementtype{filename} A list of filenames to load,
 *                                          in locale encoding.
 *  @param readonly Whether to open the document in read-only mode.
 *  @param ft @nullable The filetype for the document or @c NULL
 *                      to auto-detect the filetype.
													...
													GeanyDocument *doc)
{
	...
	{
		/* this condition cannot be reached via info bar buttons, but
		 * by our code to replace this bar with a higher priority one */
 ...
 *  @param forced_enc @nullable The file encoding to use or @c NULL to auto-detect
 *                              the file encoding.
 *
...
gboolean document_reload_force(GeanyDocument *doc, const gchar *forced_enc)
{
	...
	if (doc->priv->info_bars[MSG_TYPE_RESAVE] != NULL)
		gtk_info_bar_response(GTK_INFO_BAR(doc->priv->info_bars[MSG_TYPE_RESAVE]),
							  GTK_RESPONSE_CANCEL);
	
	/* Use cancel because the response handler would call this recursively */
	if (doc->priv->info_bars[MSG_TYPE_RELOAD] != NULL)
		gtk_info_bar_response(GTK_INFO_BAR(doc->priv->info_bars[MSG_TYPE_RELOAD]),
							  GTK_RESPONSE_CANCEL);
	
	/* try to set the cursor to the position before reloading */
	gint pos = sci_get_current_position(doc->editor->sci);
	GeanyDocument *new_doc = document_open_file_full(doc, NULL, pos, doc->readonly,
													 doc->file_type, forced_enc);
	
	if (file_prefs.keep_edit_history_on_reload &&
		file_prefs.show_keep_edit_history_on_reload_msg)
	{
		GtkWidget *bar = document_show_message(doc, GTK_MESSAGE_INFO,
							on_keep_edit_history_on_reload_response,
							GTK_STOCK_OK, GTK_RESPONSE_ACCEPT,
							_("Discard history"), GTK_RESPONSE_NO, NULL, 0,
							_("The buffer's previous state is stored in the history and "
							  "undoing restores it. You can disable this by discarding "
							  "the history upon reload. This message will not be "
							  "displayed again but your choice can be changed in the "
							  "various preferences."),
...
gboolean document_reload_prompt(GeanyDocument *doc, const gchar *forced_enc)
{
	...
	gboolean prompt = !file_prefs.keep_edit_history_on_reload &&
						(doc->changed || document_can_undo(doc) ||
									  ...
									  const gchar *locale_filename)
{
	...
	get_mtime(locale_filename, &doc->priv->mtime); /* get the modification time
													  from file and keep it */
#endif
}
...
static void get_line_column_from_pos(GeanyDocument *doc, guint byte_pos,
									 gint *line, gint *column)
{
	...
	*column = byte_pos - line_start;
	...
	/* any non-ASCII characters are encoded with two bytes(UTF-8, always in Scintilla),
	 * so skip one byte(i++) and decrease the column number which is based on byte count */
	for (gint i = line_start; i < (line_start + *column); i++)
	{
...
static void replace_header_filename(GeanyDocument *doc)
{
	...
	gchar *filebase = g_regex_escape_string(GEANY_STRING_UNTITLED, -1);
	...
	if (search_find_text(doc->editor->sci, GEANY_FIND_MATCHCASE |
										   GEANY_FIND_REGEXP,
						 &ttf, NULL) != -1)
	{
...
/**
 *  Renames the file in @a doc to @a new_filename. Only the file on disk
 *  is actually renamed, you still have to call @ref document_save_file_as()
 *  to change the @a doc object.
 *  It also stops monitoring for file changes to prevent receiving
...
{
	/* do not call queue_colourise because to we want
	 * to keep the text-changed indication! */
...
static void unprotect_document(GeanyDocument *doc)
{
	...
	if (!--doc->priv->protected && !doc->readonly)
...
gboolean document_save_file_as(GeanyDocument *doc, const gchar *utf8_fname)
{
	...
	gboolean new_file = document_need_save_as(doc) ||
							(utf8_fname != NULL &&
	...
	/* file monitoring support, add file monitoring after the file
	 * has been saved to ignore any earlier events */
	monitor_file_setup(doc);
									  ...
									  gchar **data, gsize *len)
{
	...
	gsize bytes_read, conv_len;
	
	/* try to convert it from UTF-8 to original encoding */
	gchar *conv_file_contents = g_convert(*data, *len - 1, doc->encoding, "UTF-8",
										  &bytes_read, &conv_len, &conv_error);
	if (conv_error != NULL)
	{
		gchar *text = g_strdup_printf(_("An error occurred while converting the file "
										"from UTF-8 in \"%s\". The file remains unsaved."),
									  doc->encoding);
		gchar *error_text;
		...
		if (conv_error->code == G_CONVERT_ERROR_ILLEGAL_SEQUENCE)
		{
			...
			/* take only one valid Unicode character from the context
			...
			error_text = g_strdup_printf(_("Error message: %s\nThe error occurred "
										   "at \"%s\" (line: %d, column: %d)."),
										 conv_error->message, context,
										 line + 1, column);
		}
		else
			error_text = g_strdup_printf(_("Error message: %s."),
								 ...
								 const gchar *data, gsize len)
{
	...
	{
		/* Use old GLib API for safe saving
		 * (GVFS-safe, but alters ownership and permissons).
	...
	else if (USE_GIO_FILE_OPERATIONS)
	{
		 ...
		 * It is best in most GVFS setups but don't seem to work correctly
		 * on some more complex setups (saving from some VM to their host,
		 * over some SMB shares, etc.) */
		GFile *fp = g_file_new_for_path(locale_filename);
		g_file_replace_contents(fp, data, len, NULL,
								file_prefs.gio_unsafe_save_backup,
	...
	else
	{
		...
		errno = 0;
		FILE *fp = g_fopen(locale_filename, "wb");
		if (fp == NULL)
		{
			save_errno = errno;
		...
		{
			errno = 0;
			gsize bytes_written = fwrite(data, sizeof(gchar), len, fp);
					   ...
					   const gchar *data, gsize len)
{
	...
	gchar *err = write_data_to_disk(locale_filename, data, len);
	if (err) return err;
...
static gboolean save_file_handle_infobars(GeanyDocument *doc, gboolean force)
{
	...
	if (doc->priv->info_bars[MSG_TYPE_RELOAD])
	{
		if (!dialogs_show_question_full(NULL, _("_Overwrite"), GTK_STOCK_CANCEL,
										_("Overwrite?"),
										_("The file '%s' on the disk is more "
										  "recent than the current buffer."),
										doc->file_name))
			return FALSE;
 ...
 *  If the file is not modified, this function may do nothing
 *  unless @a force is set to @c TRUE.
 *
 *  Saving may include replacing tabs with spaces,
 *  stripping trailing spaces and adding a final new line at the end of the file,
 *  depending on user preferences. Then the @c "document-before-save" signal
 *  is emitted, allowing plugins to modify the document before it is saved,
 ...
 *
 *  @warning You should ensure @c doc->file_name has an absolute path unless you
 *  want the Save As dialog to be shown. A @c NULL value also shows the dialog.
 *  This behaviour was added in Geany 1.22.
 *
 *  @param doc The document to save.
 *  @param force Whether to save the file even if it is not modified.
 *
 *  @return @c TRUE if the file was saved or @c FALSE if the file could not
...
gboolean document_save_file(GeanyDocument *doc, gboolean force)
{
	...
	{	/* always write a UTF-8 BOM because in this moment the text itself
		 * is still in UTF-8 encoding, it will be converted to doc->encoding
		 * below and this conversion also changes the BOM */
	...
	gchar *errmsg = save_doc(doc, locale_filename, data, len);
	g_free(data);
								  ...
								  gboolean inc, gboolean backwards)
{
	...
	gint start_pos = (inc || backwards) ? sci_get_selection_start(doc->editor->sci)
	...
	gint search_pos = sci_find_text(doc->editor->sci, 0, &ttf);
	...
	if (search_pos != -1)
	{
		gint line = sci_get_line_from_position(doc->editor->sci,
											   ttf.chrgText.cpMin);
		...
		{	/* we need to force scrolling in case the cursor is outside of the
			 * current visible area GeanyDocument::scroll_percent doesn't work
			 * because sci isn't always updated while searching */
						...
						gboolean scroll, GtkWidget *parent)
{
	...
	gint selection_start = sci_get_selection_start(doc->editor->sci);
	gint selection_end = sci_get_selection_end(doc->editor->sci);
	if ((selection_end - selection_start) > 0)
	{	/* there's a selection so go to the end */
		sci_goto_pos(doc->editor->sci,
					 search_backwards ? selection_start : selection_end,
					 TRUE);
	}
	
	sci_set_search_anchor(doc->editor->sci);
	
	gint search_pos = search_backwards
						? search_find_prev(doc->editor->sci, text, flags, match_)
						: search_find_next(doc->editor->sci, text, flags, match_);
	if (search_pos != -1)
	{
		/* unfold maybe folded results */
		sci_ensure_line_is_visible(doc->editor->sci,
								   sci_get_line_from_position(doc->editor->sci,
															  search_pos));
	...
	else
	{
									   ...
									   original_text))
		{
			sci_set_current_position(doc->editor->sci,
									 search_backwards ? sci_len : 0, FALSE);
			gint ret = document_find_text(doc, text, original_text, flags,
										  search_backwards, match_, scroll, parent);
			if (ret == -1)
 ...
 * @param original_find_text Text to find as it was entered by user,
 *                           or @c NULL to use @c find_text
 */
...
{
	g_return_val_if_fail(doc != NULL && find_text != NULL &&
						 replace_text != NULL, -1);
	...
	if (selection_end == selection_start)
	{
		...
		document_find_text(doc, find_text, original_find_text, flags,
						   search_backwards, NULL, TRUE, NULL);
		return -1;
	}
	/* there's a selection so go to the start before finding to search
	 * through it this ensures there is a match */
	sci_goto_pos(doc->editor->sci,
				 search_backwards ? selection_end : selection_start,
				 TRUE);
	
	GeanyMatchInfo *match = NULL;
	gint search_pos = document_find_text(doc, find_text, original_find_text, flags,
										 search_backwards, &match, TRUE, NULL);
	/* return if the original selected text did not match
	...
	if (search_pos != -1)
	{
		gint replace_len = search_replace_match(doc->editor->sci,
												match, replace_text);
...
static void show_replace_summary(GeanyDocument *doc, gint count,
								 const gchar *original_find_text,
								 const gchar *original_replace_text)
{
	...
	gchar *filename = g_path_get_basename(DOC_FILENAME(doc));
	ui_set_statusbar(TRUE,
					 ngettext("%s: replaced %d occurrence of \"%s\" with \"%s\".",
							  "%s: replaced %d occurrences of \"%s\" with \"%s\".",
							  count),
					 filename, count, original_find_text, original_replace_text);
	g_free(filename);
}
...
static guint document_replace_range(GeanyDocument *doc, const gchar *find_text,
									const gchar *replace_text, GeanyFindFlags flags,
									gint start, gint end, gboolean scroll_to_match,
									gint *new_range_end)
{
	if (new_range_end != NULL) *new_range_end = -1;
	...
	gint count = search_replace_range(sci, &ttf, flags, replace_text);
...
void document_replace_sel(GeanyDocument *doc, const gchar *find_text,
						  ...
						  GeanyFindFlags flags)
{
	g_return_if_fail(doc != NULL && find_text != NULL &&
					 replace_text != NULL);
	if (!*find_text) return;
	...
	gint selection_mode = sci_get_selection_mode(doc->editor->sci);
	gint selected_lines = sci_get_lines_selected(doc->editor->sci);
	...
	if (selection_mode == SC_SEL_RECTANGLE && selected_lines > 1)
	{
		...
		for (line = first_line; line < (first_line + selected_lines); line++)
		{
			...
			if (line_start != INVALID_POSITION)
			{
				...
				count += document_replace_range(doc, find_text, replace_text,
				...
				if (new_sel_end != -1)
				{
					...
					max_column = MAX(max_column, new_sel_end -
										sci_get_position_from_line(doc->editor->sci, line));
				}
			}
		}
	...
	{	/* update the selection for the new endpoint */
		if (selection_mode == SC_SEL_RECTANGLE && selected_lines > 1)
		{
			/* now we can scroll to the selection and destroy it because
			 * we rebuild it later */
			/*sci_goto_pos(doc->editor->sci, selection_start, FALSE);*/
			
			/* Note: the selection will be wrapped to last_line + 1 if max_column
			 * is greater than the highest column on the last line. The wrapped
			 * selection is completely different from the original one, so skip
			...
			{	/* for keeping and adjusting the selection in multi line rectangle
				 * selection we need the last line of the original selection and
				 * the greatest column number after replacing and set the
				 * selection end to the last line at the greatest column */
...
{
	g_return_val_if_fail(doc != NULL && find_text != NULL &&
						 replace_text != NULL, FALSE);
	if (!*find_text) return FALSE;
	
	gint len = sci_get_length(doc->editor->sci);
	gint count = document_replace_range(doc, find_text, replace_text,
								   flags, 0, len, TRUE, NULL);
	
	show_replace_summary(doc, count, original_find_text,
...
void document_update_tags(GeanyDocument *doc)
{
	...
	if (!doc->file_name || !doc->file_type ||
	...
	if (!doc->tm_file)
	{
		...
		const gchar *name = tm_source_file_get_lang_name(doc->file_type->lang);
	...
	gsize len = sci_get_length(doc->editor->sci);
	guchar *buffer_ptr = (guchar *)SSM(doc->editor->sci,
...
void document_highlight_tags(GeanyDocument *doc)
{
	...
	GString *keywords_str = symbols_find_typenames_as_string(doc->file_type->lang,
															 FALSE);
	if (keywords_str)
	{
...
void document_update_tag_list_in_idle(GeanyDocument *doc)
{
	...
	doc->priv->tag_list_update_source =
					g_timeout_add_full(G_PRIORITY_LOW,
									   editor_prefs.autocompletion_update_freq,
									   on_document_update_tag_list_idle, doc, NULL);
}
...
void document_set_filetype(GeanyDocument *doc, GeanyFiletype *type)
{
	...
	gboolean ft_changed = (doc->file_type != type); /* filetype has changed */
 ...
 *  This function only set the encoding of the %document, it does not
 *  any conversions. The new encoding is used when e.g. saving the file.
 *
 *  @param doc The document to use.
...
void document_set_encoding(GeanyDocument *doc, const gchar *new_encoding)
{
	...
	ui_update_statusbar(doc, -1);
	ui_widget_set_sensitive(main_widgets.window, "menu_write_unicode_bom1",
 ...
 * not be called directly, generally (use document_undo_add() instead),
 * but is used by document_redo() in order not to erase the redo stack
 * while moving an action from the redo stack to the undo stack. */
void document_undo_add_internal(GeanyDocument *doc, guint type,
								gpointer data)
{
	g_return_if_fail(doc != NULL);
	
	undo_action *action = g_new0(undo_action, 1);
	action->type = type;
	action->data = data;
...
{
	doc->changed = (sci_is_modified(doc->editor->sci) ||
					doc->has_bom != doc->priv->saved_encoding.has_bom ||
					!utils_str_equal(doc->encoding,
									 doc->priv->saved_encoding.encoding));
...
void document_undo(GeanyDocument *doc)
{
	...
	undo_action *action = g_trash_stack_pop(&doc->priv->undo_actions);
	...
	else
	{
		switch (action->type)
		{
			...
			case UNDO_EOL:
			{
				...
				document_redo_add(doc, UNDO_EOL,
			...
			case UNDO_RELOAD:
			{
				...
				/* We reuse 'data' for the redo action, so read the
				 * current EOL mode into it before proceeding. */
				data->eol_mode = editor_get_eol_char_mode(doc->editor);
				
				/* Undo the rest of the actions which are part of the reloading process. */
				for (guint i = 0; i < data->actions_count; i++)
					document_undo(doc);
...
void document_redo(GeanyDocument *doc)
{
	...
	undo_action *action = g_trash_stack_pop(&doc->priv->redo_actions);
	...
	else
	{
		switch (action->type)
		{
			case UNDO_SCINTILLA:
			{
				...
				undo_action *next_action =
								g_trash_stack_peek(&doc->priv->redo_actions);
...
/* returns an identifier that is to be set as a widget name or class to get
 * it styled depending on the document status (changed, readonly, etc.)
 * a NULL return value means default (unchanged) style */
const gchar *document_get_status_widget_class(GeanyDocument *doc)
{
	g_return_val_if_fail(doc != NULL, NULL);
	
	gint status = document_get_status_id(doc);
	
	return (status < 0) ? NULL : document_status_styles[status].name;
}


/**
 *  Gets the status color of the document, or @c NULL if default widget
 *  coloring should be used.
 *  Returned colors are red if the document has changes, green if the document
 *  is read-only or simply @c NULL if the document is unmodified but writable.
 *
 *  @param doc The document to use.
 *
 *  @return @nullable The color for the document or @c NULL if the default
...
const GdkColor *document_get_status_color(GeanyDocument *doc)
{
	...
	if (!document_status_styles[status].loaded)
	{
		...
		gtk_widget_path_iter_set_name(path, -1,
				document_status_styles[status].name);
		gtk_style_context_set_screen(ctx,
				gtk_widget_get_screen(GTK_WIDGET(doc->editor->sci)));
		gtk_style_context_set_path(ctx, path);
		gtk_style_context_get_color(ctx,
				gtk_style_context_get_state(ctx), &color);
...
GeanyDocument *document_clone(GeanyDocument *old_doc)
{
	...
	ScintillaObject *old_sci = old_doc->editor->sci;
	
	gchar *text = sci_has_selection(old_sci) ? sci_get_selection_contents(old_sci)
											 : sci_get_contents(old_sci, -1);
	
	GeanyDocument *doc = document_new_file(NULL, old_doc->file_type, text);
	g_free(text);
	document_set_text_changed(doc, TRUE);
...
{
	guint page_count = gtk_notebook_get_n_pages(GTK_NOTEBOOK(main_widgets.notebook));
	
	/* iterate over documents in tabs order */
	for (guint p = 0; p < page_count; p++)
	{
...
{
	va_list args;
	GtkWidget *parent = document_get_notebook_child(doc);
	
	va_start(args, format);
	gchar *text = g_strdup_vprintf(format, args);
	va_end(args);
	
	gchar *markup = g_markup_printf_escaped("<span size=\"larger\">%s</span>", text);
	g_free(text);
	
	GtkWidget *info_widget = gtk_info_bar_new();
	...
	GtkWidget *content_area = gtk_info_bar_get_content_area(GTK_INFO_BAR(info_widget));
	
	GtkWidget *label = geany_wrap_label_new(NULL);
...
static void on_monitor_reload_file_response(GtkWidget *bar, gint response_id,
											GeanyDocument *doc)
{
...
static gboolean on_sci_key(GtkWidget *widget, GdkEventKey *event,
						   gpointer data)
{
	...
	switch (event->keyval)
	{
		...
		case GDK_ISO_Left_Tab:
		{
			...
			GtkDirectionType dir = event->keyval == GDK_Tab ? GTK_DIR_TAB_FORWARD
...
static void enable_key_intercept(GeanyDocument *doc, GtkWidget *bar)
{
	...
	g_signal_connect_object(bar, "destroy", G_CALLBACK(gtk_widget_grab_focus),
							doc->editor->sci, G_CONNECT_SWAPPED);
	g_signal_connect_object(doc->editor->sci, "key-press-event",
							G_CALLBACK(on_sci_key), bar, 0);
}
...
/* Set force to force a disk check, otherwise it is ignored if there
 * was a check in the last file_prefs.disk_check_timeout seconds.
...
gboolean document_check_disk_status(GeanyDocument *doc, gboolean force)
{
	...
	gboolean use_gio_filemon = (doc->priv->monitor != NULL);
	...
	else
	{
		...
		if (!force && doc->priv->last_check >
				(cur_time - file_prefs.disk_check_timeout))
			return FALSE;
...
gint document_compare_by_display_name(gconstpointer a, gconstpointer b)
{
	...
	gchar *base_name_a = g_path_get_basename(DOC_FILENAME(doc_a));
	gchar *base_name_b = g_path_get_basename(DOC_FILENAME(doc_b));
	
	gint result = strcmp(base_name_a, base_name_b);
...
gint document_compare_by_tab_order(gconstpointer a, gconstpointer b)
{
	...
	gint notebook_position_doc_a = document_get_notebook_page(doc_a);
	gint notebook_position_doc_b = document_get_notebook_page(doc_b);
