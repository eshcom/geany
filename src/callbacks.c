...
/*
 * Callbacks used by Glade. These are mainly in response to menu item
 * and button events in the main window. Callbacks not used by Glade
 * should go elsewhere.
 */
...
/* represents the state at switching a notebook page(in the left treeviews widget),
 * to not emit the selection-changed signal from tv.tree_openfiles */
...
{
	guint max = (guint)gtk_notebook_get_n_pages(
...
void on_paste1_activate(GtkMenuItem *menuitem, gpointer user_data)
{
	...
	{
		GtkTextBuffer *buffer = gtk_text_view_get_buffer(GTK_TEXT_VIEW(focusw));
		gtk_text_buffer_paste_clipboard(buffer, gtk_clipboard_get(GDK_NONE),
										NULL, TRUE);
	}
}
...
static void do_toolbar_search(const gchar *text, gboolean incremental,
							  gboolean backwards)
{
	GeanyDocument *doc = document_get_current();
...
static void on_tv_notebook_switch_page(GtkNotebook *notebook, gpointer page,
									   guint page_num, gpointer user_data)
{
	/* suppress selection changed signal when switching
	 * to the open files list */
...
static void convert_eol(gint mode)
{
	...
	document_undo_add(doc, UNDO_EOL,
							...
							gpointer user_data)
{
	if (event->button == 3)
	{
		gtk_menu_popup(GTK_MENU(ui_widgets.toolbar_menu), NULL, NULL, NULL, NULL,
...
void on_toggle_case1_activate(GtkMenuItem *menuitem, gpointer user_data)
{
	...
	if (sci_has_selection(sci))
	{
		...
		if (result != NULL)
		{
			...
			g_free(result);
			if (keep_sel)
				sci_set_selection_start(sci, sci_get_current_position(sci) -
															strlen(text));
		}
		else
...
void on_show_color_chooser1_activate(GtkMenuItem *menuitem, gpointer user_data)
{
	...
	editor_find_current_word(doc->editor, pos, colour, sizeof colour,
							 GEANY_WORDCHARS"#");
...
{
	keybindings_send_command(GEANY_KEY_GROUP_BUILD,
...
void on_find_previous1_activate(GtkMenuItem *menuitem, gpointer user_data)
{
	if (search_data.flags & GEANY_FIND_REGEXP)
		/* Can't reverse search order for a regex
		 * (find next ignores search backwards) */
		utils_beep();
	else
...
void on_go_to_line_activate(GtkMenuItem *menuitem, gpointer user_data)
{
	...
	gchar *result = dialogs_show_input_goto_line(
...
{
	gchar *uri = utils_get_help_url(NULL);
...
static void on_comments_function_activate(GtkMenuItem *menuitem,
										  gpointer user_data)
{
	GeanyDocument *doc = document_get_current();
	if (doc == NULL || doc->file_type == NULL)
	{
		ui_set_statusbar(FALSE, _("Please set the filetype for the current "
...
{
	insert_comment_template(document_get_current(), editor_info.click_pos,
							GEANY_TEMPLATE_GPL);
}
										   ...
										   gpointer user_data)
{
	...
	gchar *text = templates_get_template_changelog(doc);
	sci_start_undo_action(doc->editor->sci);
	...
	sci_goto_pos(doc->editor->sci, 21 + strlen(template_prefs.developer)
									  + strlen(template_prefs.mail), TRUE);
	sci_end_undo_action(doc->editor->sci);
	g_free(text);
}
											...
											gpointer user_data)
{
	...
	const gchar *fname = doc->file_name;
	gchar *text = templates_get_template_fileheader(FILETYPE_ID(ft), fname);
								   ...
								   gpointer user_data)
{
	...
	if (!ui_prefs.sidebar_visible
		&& gtk_container_get_focus_child(
				GTK_CONTAINER(main_widgets.sidebar_notebook)) != NULL)
		keybindings_send_command(GEANY_KEY_GROUP_FOCUS,
								 GEANY_KEYS_FOCUS_EDITOR);
...
void on_context_action1_activate(GtkMenuItem *menuitem, gpointer user_data)
{
	...
	gchar *word = sci_has_selection(doc->editor->sci)
						/* take selected text if there is a selection */
						? sci_get_selection_contents(doc->editor->sci)
						: g_strdup(editor_info.current_word);
	gchar *command;
	const gchar *check_msg;
	
	/* use the filetype specific command if available,
	 * fallback to global command otherwise */
	if (doc->file_type != NULL &&
									  ...
									  gpointer user_data)
{
	if (event->changed_mask & GDK_WINDOW_STATE_FULLSCREEN)
	{
		...
		ui_prefs.fullscreen = (event->new_window_state &
							   GDK_WINDOW_STATE_FULLSCREEN) ? TRUE : FALSE;
...
static void on_delete_current_lines1_activate(GtkMenuItem *menuitem,
											  gpointer user_data)
{
	keybindings_send_command(GEANY_KEY_GROUP_EDITOR,
...
static void on_insert_alternative_white_space1_activate(GtkMenuItem *menuitem,
														gpointer user_data)
{
	keybindings_send_command(GEANY_KEY_GROUP_INSERT,
...
{
	keybindings_send_command(GEANY_KEY_GROUP_SEARCH,
								 ...
								 GConnectFlags flags, gpointer user_data)
{
	...
	GCallback callback = g_hash_table_lookup(hash, handler_name);
